...
**Version:** 35.29.0 (2026-06-27) — account creation realigned to the reference implementation: `TransferActivation` (opcode 0x0A) is the sponsor account-creation operation; `Transfer` (0x02) is top-up only (receiver must exist); AccountRecord field `last_activation_window`, reject `ActivationCooldownNotElapsed`; spec now byte-exact to the code's model
...
**Step 0: Account user (entering the network).** Holds an account keypair (in a smartphone, hardware wallet, any client). Connects to a node in the network — their own or someone else's — through the network's transport layer (level 3). The account record appears in the Account Table upon the first incoming `TransferActivation` from an existing account (the AccountRecord is created atomically together with crediting the amount); explicit creation is not required. Uses the network: Montana transfers, data commitment via `Anchor`. All other services (voice, video, premium features, data storage, name resolution, creator subscriptions) live at the application layer through direct Montana transfers between accounts. No earnings at the protocol layer. Barrier to entry: a smartphone + a first incoming transfer (any positive amount from an existing account).

**Step 1: Node operator (earnings).** Runs their own node 24/7 + an operator account bound to the node (+ optionally additional personal accounts). Maximum sovereignty: data on the operator's own hardware, full participation in consensus, earnings through the node lottery (the per-window reward, see the section on the Montana currency). Barrier: commodity hardware (at least one core), 24/7 uptime, a network connection, and a sequential SHA-256 chain of length `ssha_chain_length × D` iterations at node registration.
...
- **Domain separators** (`"mt-op"`, `"mt-proposal"`, `"mt-bundle"`, `"mt-ssha-reveal"`, `"mt-lottery"`, `"mt-bc-aggregate"`, `"mt-selection"`, etc.) — only in the «Consensus encoding layer», «Domain separators registry». All formulas refer to the domain by name from the registry; they do not duplicate the literal string under a new name.
...
- **Object structures** (layouts for Proposal header, BundledConfirmation, SSHA_Reveal, NodeRegistration, UserObjects, Account / Node / Candidate records) — one authoritative layout block + one `**Invariants X:**` section (per Gate 13). Illustrative ASCII diagrams do NOT contain type annotations (per Gate 13c — the architect-role section).
...
1. **Sequential time barrier.** Creating the record requires a sequential SHA-256 iteration count integer-specified in the Genesis Decree (for example `ssha_chain_length × D` SHA-256 hashes for NodeRegistration). Sequential time is a non-acquirable scarcity, symmetric for all participants. Applicable to validator-class records where the sequential cost is justified by the target throughput.
...
Rationale: Sybil on voting / lottery is closed by chain_length-weighted mechanisms (nodes) and activity-based pruning (accounts), but this does not address resource consumption through fan-out. A million accounts does not change the distribution of lottery weights but occupies ×million `AccountRecord` entries in the state trie. The time-based cooldown for AccountRecord creation `1 TransferActivation per sender per τ₂` for user accounts and the sequential SHA-256 chain `ssha_chain_length × D` for node candidacies together close both vectors via canonical time-based primitives.
...
| `NodeTable`            | [I-14].1 sequential time barrier (NodeRegistration sequential SHA-256 chain `ssha_chain_length × D`) + [I-14].2 activity-based (inactivity prune 8τ₂) + [I-14].3 hard quota (`selection_interval` 336 windows, admission ≤1% active per event) | closed       |
...
Operation создающая запись:           TransferActivation (sponsor → новый receiver) либо
...
Time-bound sabotage anaylsis:
  Bytes per τ₂ от 1 sender (TransferActivation):  2 059 B (cooldown 1 TransferActivation per sender per τ₂)
...
Платит creation cost:                 n/a ([I-15] denies cost-based barrier; sequential SSHA — time barrier)
...
Cost per record:                      n/a (no fee; cost — sequential SSHA τ₂ окон ~ две недели CPU)
Time-bound sabotage analysis:
  ...
  Pre-condition per slot:             SSHA τ₂ окон последовательного хэширования за каждого кандидата
                                      (sequential SHA-256, не parallelizable, не free)

Sabotage asymmetry:                   в пользу сети (admission rate 1% active per event = bounded
                                      growth; sequential SSHA — non-amortizable cost; `inactivity prune
...
[I-14] путь:                          combo (1 + 2 + 3) — sequential SSHA + activity-based pruning +
...
Платит creation cost:                 n/a ([I-15]; cost — sequential SSHA + opportunity cost)
...
Cost per record:                      n/a
Time-bound sabotage analysis:
  Bytes per τ₂ от 1 actor:            ≤τ₂_windows / ssha_entry_windows × 4 034 B = 1 × 4 034 B
                                      (один кандидат за τ₂ при baseline SSHA; adaptive SSHA
  ...
  Pre-condition:                      каждый candidate требует τ₂ окон sequential SSHA + ML-DSA-65
...
Sabotage asymmetry:                   в пользу сети (3τ₂ TTL — explicit auto-deletion; admission rate
                                      ...
                                      закрыт через cemented_bundle_aggregate в candidate_ssha_init)
...
Дефицитный ресурс протокола — **время**: SSHA-непрерывность TimeChain, τ-окна, chain_length узла, activity аккаунта, sequential candidate SSHA. Этот time-market встроен в консенсус как единственный объективный дефицит. Защиты через существующий дефицит (а) симметричны для всех участников независимо от Ɉ-holdings, (б) не дублируют логику существующих time-based ограничителей консенсуса, (в) не зависят от номинальной стоимости Ɉ.
...
- **Sequential SSHA iteration count** — кандидатура узла требует sequential SHA-256 цепочки длины `ssha_chain_length × D` хэшей (существующая NodeRegistration защита, [Sybil-защита Шаг 2 + Adaptive SSHA]). Sequential time — единственный неприобретаемый дефицит.
...
Различающий критерий: проблема «кто-то создаёт много записей потребляющих сетевые ресурсы без legitimate use» → time-based defenses (rate-per-identity, cooldown, TTL); проблема «кто-то претендует на роль валидатора без вложенного времени» → sequential SSHA iteration count + chain_length-weighted влияние на консенсус. Аппликативные платежи — задача прикладного слоя поверх `Transfer`, не protocol-level concern.
...
**Paternы онлайн-активности оператора — видны.** Подписи BundledConfirmation и SSHA_Reveal публичны. Оператор, подписывающий bundles, раскрывает свой рабочий график. Для оператора-активиста это наблюдаемо.
...
- **Canonical window count.** Синхронен между узлами через SSHA chain length. Узлы быстрее median простаивают между окнами; узлы медленнее median отстают и догоняют через SSHA computation. Никакой узел не сообщает свою wall-clock длительность в сеть.
...
Для любого подписанного объекта протокола (UserObject, ControlObject, Proposal header, BundledConfirmation, SSHA_Reveal, любой future-вводимый подписанный класс) действуют три универсальных правила.
...
| SSHA_Reveal | `NodeTable[node_id].suite_id` |
...
| SSHA_Reveal | `"mt-ssha-reveal"` | identifier (SHA-256 hash) |
...
Type byte `0x01` **не выделен**. AccountRecord создаётся: для аккаунта пользователя — операцией `TransferActivation` (0x0A) от существующего аккаунта-спонсора; для operator-аккаунта — атомарно с записью узла в Node Table при cementing Selection event для NodeRegistration. Самоинициация создания аккаунта невозможна — требуется существующий спонсор.
...
- SSHA_Reveal — `"mt-ssha-reveal"` class domain
...
**TransferActivation** — спонсорская активация: существующий аккаунт (sender-спонсор) создаёт новый AccountRecord для receiver и зачисляет `amount`. Единственный путь появления user-аккаунта в сети; самоинициация невозможна — нужен существующий спонсор.
...
- **Cooldown** per [I-15]: `current_window >= Account Table[sender].last_activation_window + τ₂_windows` (sender выполняет максимум одну `TransferActivation` за τ₂; нарушение — **reject** `ActivationCooldownNotElapsed`). Исключение — sender с `last_activation_window == 0` (никогда не активировал) проходит без проверки.
- Signature ML-DSA-65 valid для `Account Table[sender].current_pubkey` над signed_scope (Правило R1)
...
- `Account Table[payload.receiver] = new_record(balance = payload.amount, current_pubkey = payload.receiver_pubkey, suite_id = payload.suite_id, frontier_hash = 0x00...00, op_height = 0, account_chain_length = 0, account_chain_length_snapshot = 0, last_activation_window = 0, is_node_operator = 0, creation_window = current_window, last_op_window = current_window)`
...
Публичное (верифицируемо всеми):
  ...
  SSHA:               TimeChain values, lottery endpoints, подписи
```
...
Candidate Pool (запись на кандидата):
  ...
  proof_endpoint                   32B     <- endpoint SSHA цепочки (длина ssha_chain_length)
  W_start                          8B     <- u64, окно начала SSHA (заявлено кандидатом)
  ssha_chain_length                 8B     <- u64, длина SSHA цепочки от candidate_ssha_init до proof_endpoint (в "окнах" по D хэшей)
...
included_reveals_root: ключ = reveal_author_node_id (signer SshaReveal,
                       canonical из NodeTable);
...
TimeChain — глобальные часы (ход времени, SSHA). NodeChain — присутствие узла (последовательность cemented BundledConfirmation). AccountChain — присутствие аккаунта (дискретные операции). AccountTable — состояние счёта.

### TimeChain SSHA — осциллятор

Конкретный subtype: **iterated SHA-256 chain** — `T_r = SHA-256(T_{r-1})`, sequential by construction. Верификация требует пересчёта O(D × chain_length) последовательных SHA-256 (нет succinct proof размером O(log T) как у Wesolowski ePrint 2018/623 или Pietrzak ePrint 2018/627). Выбор подтипа обоснован [I-1]: production-grade succinct SSHA существуют только на classical groups (RSA, class groups мнимо-квадратичных полей) — ломаются Shor's algorithm; PQ-secure succinct SSHA на момент Genesis — research grade без production audit history.
...
Доказательство присутствия конкретного node_id в каждом окне. Каждое окно с cemented BundledConfirmation = одно звено NodeChain. chain_length — позиция узла в NodeChain: = 1 при активации через selection event, +1 при каждом cemented BundledConfirmation. Инвариант: chain_length ≥ 1 для любого узла в Node Table — гарантирует корректность знаменателей в weighted_ticket лотереи и в seniority_term.

NodeChain не является SSHA-цепочкой. Узел доказывает присутствие публикацией BundledConfirmation (подтверждение операций сети), не вычислением per-node SSHA. Один SSHA на всю сеть (TimeChain) — достаточен.
...
| Защита от Sybil | τ₂ окон SSHA + selection event | накопление окон требует активности |
...
### SSHA Reveal и лотерея

В лотерее участвует **только один класс** субъектов — узлы (через SSHA_Reveal). Аккаунты в лотерее не участвуют (см. раздел «Аккаунты не участвуют в лотерее» ниже). Каждый узел производит ticket, взвешенный по длине своей NodeChain.

Confirmers (~100 узлов с наибольшим chain_length) публикуют BundledConfirmation для финализации окна. Все узлы с weighted_ticket_node < target публикуют SSHA_Reveal для лотереи. SSHA_Reveal цементируется через BundledConfirmation: confirmers включают полученные SSHA_Reveals в свои bundles наряду с UserObjects и ControlObjects. Cement threshold тот же — 67% active_chain_length. Proposer извлекает только cemented reveals — дискреция над лотереей = ноль.

#### Класс 1: узлы

После завершения SSHA окна W каждый узел вычисляет свой ticket.
...
Где:
- `T_r(W)` — TimeChain SSHA output окна W (каноничен, одинаков у всех узлов).
...
**Grinding resistance.** Атакующий с SSHA hardware advantage способен пре-вычислить `T_r(W)` на много окон вперёд. Но `cemented_bundle_aggregate(W-2)` содержит ML-DSA-65 подписи будущих confirmers — их privкey не у атакующего, aggregate непредсказуем offline. Grinding по node_id (выбор keypair с favorable future endpoints) не работает: endpoint зависит от canonical-но-непредсказуемого компонента. Горизонт grinding схлопывается до уже cemented (публично известного) окна W-2, где keypair уже зафиксирован.

Если weighted_ticket_node < target — узел кандидат и публикует SSHA_Reveal:
...
`reveal_hash` = `identifier(reveal)` с class domain `"mt-ssha-reveal"` (Правило R2).

**Инварианты SSHA_Reveal:**
...
- Один SSHA_Reveal per `(node_id, window_index)` — повторный отклоняется (equivocation)
...
1. Окно W-1 завершается: confirmers публикуют BundledConfirmation_{W-1} (операции окна W-1 + SSHA_Reveals окна W-2), кандидаты публикуют SSHA_Reveal_{W-1}, аккаунты публикуют операции.
2. `proposer_W = winner_{W-2}` (канонически определён из proposal_{W-1}).
3. Окно W начинается. Confirmers получают SSHA_Reveals_{W-1} через P2P и включают их в BundledConfirmation_W наряду с операциями окна W. SSHA_Reveal идентифицируется по `window_index = W-1`.
4. SSHA_Reveal_{W-1} cemented когда confirmers с суммарным chain_length ≥ 67% active_chain_length включили его в свои BundledConfirmation_W. Cement status каноничен — каждый узел отслеживает его независимо по P2P bundles.
5. Proposer_W собирает BundledConfirmation-ы окна W-1 и cemented set:
   ...
   included_reveals_{W-1} = SSHA_Reveal-ы окна W-1, cemented через
...
7. `winner_{W-1} = argmin(weighted_ticket_node)` среди cemented SSHA_Reveal узлов-кандидатов окна W-1.
...
9. Сеть валидирует proposal_W:
   ...
   - included_reveals_{W-1} = cemented set SSHA_Reveals окна W-1? (валидатор сверяет с собственным tracking cement status из BundledConfirmation окна W)
...
**Cross-window cementing timeline.** SSHA_Reveals окна W-1 публикуются при завершении окна W-1 (SSHA computation = window duration). Цементируются в BundledConfirmation окна W. Между публикацией reveals и сборкой proposal — целое окно. Timing constraint отсутствует.

**Leader skin in the game.** Proposer_W публикует свой SSHA_Reveal для окна W. Если его proposal отклонён (< 67% подписей сети), его SSHA_Reveal исключается из пула кандидатов окна W. Потеря lottery ticket = экономический кнут за цензуру или бездействие. Отказ подписать proposal = implicit rejection от каждого узла.

**Genesis cold-start.** Окно 0 — пустой литерал без операторов. Пока нет активных операторов, первый кандидат, набравший требуемую последовательную работу (`required_ssha_length` при `active_chain_length = 0`), проповедует собственный приём (`selection_slots(0) = 1`) и цементирует его сам (`quorum(1) = 1`). После появления первого Active-оператора — стандартная lookback логика: `proposer_W = winner_{W-2}`.

#### Калибровка target

Target калиброван на ~13 кандидатов SSHA_Reveal за окно. Калибровка на τ₂:
...
Input:
  target_old: u128
  total_reveals_τ₂: u64      (сумма SSHA_Reveals за τ₂ окон)
...
Трафик reveal за окно: ~13 SSHA_Reveal × 738B ≈ 9.6 KB (P2P gossip; далее включаются в BundledConfirmation для cementing). Аккаунты участвуют через cemented операции в BundledConfirmation — дополнительного трафика для аккаунтов нет.
...
`cemented_bundle_aggregate(W-2)` в endpoint formula (см. раздел «SSHA_Reveal» и
...
| Тип | Описание | Валидация |
|-----|----------|-----------|
| NodeRegistration | Регистрация узла (кандидатура) | ML-DSA-65 `signature` валидна для `node_pubkey` над signed_scope (Правило R1); ML-DSA-65 `operator_pop` валидна для `operator_pubkey` над bytes (`"mt-operator-pop" \|\| node_pubkey`) — proof of possession с class domain separator (Правило R2; без него squatting на чужой operator_pubkey возможен через cross-class confusion); `node_id` уникален (не в Node Table и не в Candidate Pool); `operator_account_id == SHA-256("mt-account" \|\| suite_id \|\| operator_pubkey)` (binding derivation); если AccountRecord operator-а существует — `is_node_operator == 0` AND `current_pubkey == operator_pubkey`, иначе AccountRecord создаётся атомарно при cementing Selection event; `proof_endpoint` верифицируем через SSHA от candidate_ssha_init. `nodereg_hash` = identifier(nr) с class domain `"mt-nodereg"` (Правило R2) |

Каждый узел валидирует объекты обоих классов локально при получении. Валидные объекты ретранслируются по P2P.

Все объекты — UserObjects, ControlObjects и SSHA_Reveals — финализируются (cemented) одинаково: через 67% active_chain_length подтверждения в BundledConfirmation. Cemented status объективен и одинаков для всех узлов. Дискреция победителя над включением ControlObjects и SSHA_Reveals = ноль.
...
Window W-1:  confirmers publish BundledConfirmation_{W-1}
               (W-1 operations + W-2 SSHA_Reveals)
             SSHA_{W-1} completes → candidates publish SSHA_Reveal_{W-1}
             accounts publish cemented operations
                              │
Window W:    confirmers publish BundledConfirmation_W
               (W operations + W-1 SSHA_Reveals)
             W-1 SSHA_Reveals cemented (67% active_chain_length)
                              │
...
- **Cemented reveals.** SSHA_Reveals окна W-1 публикуются при завершении W-1, цементируются чер��з BundledConfirmation окна W (confirmers включают полученные reveals в свои bundles). `included_reveals_{W-1}` = cemented set (67% active_chain_length). Proposer извлекает cemented reveals и cemented account operations из included_bundles_{W-1}, определяет `winner_{W-1} = argmin(weighted_ticket)`. Дискреция proposer-а над составом лотереи = ноль.
- **Canonical acceptance.** Сеть валидирует proposal_W: (a) proposer = winner_{W-2}, (b) included_bundles ≥ 67% active_chain_length, (c) included_reveals = cemented set SSHA_Reveals окна W-1, (d) winner_{W-1} = argmin из (cemented reveals ∪ account_candidates), (e) state_root корректен. Если 67% active_chain_length подписывают proposal_W → cemented. Canonical set зафиксирован.
- **Leader skin in the game.** Proposer_W участвует в лотерее окна W через свой SSHA_Reveal (cemented в BundledConfirmation окна W+1). При отклонении proposal (< 67% подписей) — SSHA_Reveal proposer-а исключается из пула кандидатов окна W. Отказ подписать proposal = implicit rejection. Отдельного censorship vote нет.
...
- **included_reveals_{W-1}**: SSHA_Reveals окна W-1, cemented через BundledConfirmation окна W (67% active_chain_length). Из cemented reveals определяется `winner_{W-1}` (получатель `reward(W-1)` за окно W-1). Лотерея single-class — winner всегда узел; cemented account operations окна W-1 не участвуют в выборе winner-а.
...
Proposal header:
  ...
  included_reveals_root 32B    <- Merkle root списка SSHA_Reveal-ов окна W-1,
...
SSHA следующего окна вычисляется непрерывно, не ожидая завершения финализации предыдущего. TimeChain для окна N+1 детерминирован — каждый узел вычисляет его независимо. Reveal phase и финализация происходят параллельно с началом SSHA следующего окна.
...
Confirmer собирает все валидные объекты за окно и публикует один BundledConfirmation. Bundle содержит два класса хэшей: (1) операции текущего окна W (UserObjects + ControlObjects) и (2) SSHA_Reveals предыдущего окна W-1 (лотерейные билеты, опубликованные при завершении W-1 и полученные через P2P):
...
BundledConfirmation:
  ...
  reveal_hashes[]   reveal_count × 32B <- identifier(reveal) с class "mt-ssha-reveal" окна W-1
...
- Каждый элемент `reveal_hashes[i]` — 32B `identifier(reveal)` с class domain `"mt-ssha-reveal"` окна W-1; дубликаты **запрещены**
...
**Cemented bundle aggregate.** Канонический агрегат идентичностей confirmers окна W, используемый как unpredictable-offline компонент в формулах lottery endpoint, sort_key и candidate_ssha_init. Aggregate строится по Правилу R3 (aggregate over signer_node_id, не over signatures и не over content):
...
- **Непредсказуемый offline (в стандартной ветви).** Зависит от эмерджентного состава S_W — какие именно active confirmers набрали quorum. Атакующий с SSHA hardware advantage не может пре-вычислить будущий S_W без координированного control over honest participants (никто single confirmer не контролирует набор других cemented confirmers).
...
apply_proposal(state, proposal) -> state':
  ...
  Шаг 1: применить control_set в порядке (cemented_window asc, op_hash lex asc).
    NodeRegistration: проверить node_id уникален (нет в Node Table и Candidate Pool),
                      ...
                      проверить W_p - max_ssha_horizon ≤ W_start ≤ W_p - base_ssha_length
                        (max_ssha_horizon = 2 × τ₂_windows; base_ssha_length = τ₂_windows = ssha_entry_windows),
                      применить incremental apply в окне W_p:
                        ...
                        for each NR in sorted order:
                          ...
                          required_ssha_length(NR) = adaptive_formula(current_pressure)
                          if NR.ssha_chain_length < required_ssha_length(NR):
                            reject NR (insufficient SSHA work)
                            continue
                          верифицировать proof_endpoint: пересчёт SSHA от
                            SHA-256("mt-candidate-ssha-init" ||
                                    timechain_value(W_start) ||
                                    cemented_bundle_aggregate(W_start - 2) ||
                                    node_id)
                            через NR.ssha_chain_length окон,
                          ...
                          создать запись в Candidate Pool:
                            ...
                            proof_endpoint, W_start, ssha_chain_length,
...
**Grinding resistance selection event.** Domain separator `mt-selection` отделяет hash space от `mt-lottery` и других. Компонент `cemented_bundle_aggregate(current_window - 2)` — канонический но unpredictable offline (зависит от ML-DSA-65 подписей confirmers окна current_window-2). Атакующий с SSHA hardware advantage, пре-вычисляющий `timechain_value` для будущих selection events, не может пре-вычислить sort_key без privкey confirmers. Grinding keypair (генерация N kerpairs для выбора favorable node_id) не работает: к моменту selection event sort_key определён будущими signatures, которые атакующий не контролирует.
...
Минимум для узла: **1 ядро CPU**. TimeChain SSHA (непрерывное последовательное хэширование) + валидация операций (interleaved, overhead < 1% окна). С ростом TPS сети дополнительные ядра ускоряют верификацию операций. Один узел = 1 ядро. Любой компьютер = потенциальный узел. Верификация операций аккаунтов полностью параллелизуется — цепочки аккаунтов независимы.

### Вход и регистрация

Два уровня входа в сеть. Узлы участвуют в консенсусе — открытый вход через SSHA + selection event, AccountRecord operator-а создаётся атомарно при cementing Selection event. Аккаунты пользователей держат и переводят средства — AccountRecord появляется операцией `TransferActivation` от существующего аккаунта-спонсора (создание + зачисление amount). Самоинициация создания аккаунта невозможна — требуется существующий спонсор.

**Genesis State — пустое окно 0.** Genesis State не содержит предзаданных операторов: Account Table, Node Table и Candidate Pool пусты. Окно 0 — литеральная константа без чьей-либо identity. Первый оператор входит стандартным путём `CandidateSsha → Registered → Active`: при отсутствии активных операторов `selection_slots(0) = 1` принимает первого кандидата, набравшего требуемую последовательную работу, а `quorum(1) = 1` позволяет ему цементировать собственную цепь. Распределение Монтана новым пользователям — через `TransferActivation` с cooldown `1 per sender per τ₂` per [I-15], ограничивающим темп создания новых AccountRecord.

**Bootstrap growth model.** Genesis is empty (0 accounts). The first AccountRecord is created when the first operator is admitted by the Selection event in window 0 (`selection_slots(0) = 1`): the operator AccountRecord is created atomically with `balance = 0` (see the Selection event apply step). At `settle(1)` the winner of window 0 receives the first emission `EMISSION_moneta`, credited to that already-existing operator account. Montana then spreads through `TransferActivation` with exponential parallelism — every AccountRecord that holds a positive balance and has passed the `1 TransferActivation per τ₂` cooldown can create a new AccountRecord:

- N = 0: 0 accounts (empty genesis); the first account is the operator admitted in window 0 — created at the Selection event with `balance = 0`, then credited `EMISSION_moneta` at `settle(1)`
- N = 1 τ₂: the operator creates one new AccountRecord → 2 accounts
- N = 2 τ₂: both create one each → 4 accounts
- N = k τ₂: `2^k` accounts
...
Genesis State (до первого окна, supply = 0):
  ...
  protocol_params (каноническая сериализация, little-endian, фиксированная длина полей):
    D₀                             (8B)   начальное значение D TimeChain SSHA (325 000 000, hex 0x135F1B40)
    ...
    ssha_entry_windows              (8B)   20 160 (= τ₂)
    ...
    adaptive_ssha_threshold         (2B)   1 (= 0.01 × 100, порог давления 1%)
    adaptive_ssha_multiplier        (2B)   100 (effective_ssha = base × pressure × multiplier)
...
Runtime коррекция учитывает фактическую длительность окна на genesis-железе при SSHA-цикле под нагрузкой консенсуса (validation, gossip, BC publication interleaving): чистый бенчмарк давал ~56.35 секунд по кварцу на 305 836 793 хэшей; масштабирование до целевых 60 кварцевых секунд даёт `D₀ = 325 000 000`.
...
**Bootstrap period.** До появления второго узла (первые τ₂+ окон) первый оператор имеет 100% active_chain_length и является единственным confirmer-ом, proposer-ом и winner-ом. Это физическая необходимость запуска любой сети — кто-то является первым. Доминирование первого оператора размывается органически: каждый новый узел, прошедший selection event, вносит свой chain_length в active set. Протокольные правила (quorum 67%, weighted_ticket лотерея, selection rate limit) одинаковы с первого окна — специальных genesis-правил вне начального lookback нет.
...
- Отсутствие конкуренции — один участник, лотерея без значимых соперников, quorum тривиально достигается единственным оператором
...
**Следствия для reference implementation.** Аудит и тестирование обязаны покрывать оба режима раздельно. Тесты bootstrap-периода проверяют поведение в Genesis-режиме (single-confirmer aggregate, первый оператор выигрывает все лотереи, proposer ротация отсутствует). Тесты после bootstrap — BFT-поведение (multi-confirmer aggregate, weighted_ticket лотерея, lookback leadership). Переходный тест обязательно проверяет корректность передачи при первой non-bootstrap регистрации — один из критичных invariant-моментов в жизни сети.
...
W_start — окно начала SSHA (заявляется кандидатом в NodeRegistration).

**Шаг 2: Кандидатура.** После завершения SSHA кандидат публикует NodeRegistration:
...
NodeRegistration:
  ...
  proof_endpoint       32B     <- endpoint SSHA цепочки (длина ssha_chain_length)
  W_start               8B     <- окно начала SSHA
  ssha_chain_length      8B     <- длина SSHA цепочки в "окнах" (по D хэшей)
...
- `W_p - 2 × τ₂_windows ≤ W_start ≤ W_p - base_ssha_length` (base_ssha_length = τ₂_windows). Нижняя граница ограничивает историческое pre-computation окно. Верхняя граница гарантирует что SSHA физически выполнен до publication.
- `ssha_chain_length ≥ required_ssha_length(W_p)` — длина заявленной SSHA цепочки не меньше требуемой pressure-adjusted длины в момент W_p (incremental apply в батче, см. apply_proposal)
- `proof_endpoint` верифицируем: пересчёт SSHA от `SHA-256("mt-candidate-ssha-init" || timechain_value(W_start) || cemented_bundle_aggregate(W_start - 2) || node_id)` через `ssha_chain_length` окон. Если `ssha_chain_length × D hashes` SSHA iteration от init даёт `proof_endpoint` — валидно.

Верификация: `ssha_chain_length` сегментов SSHA проверяются параллельно. На C ядрах: ~(ssha_chain_length/C) × t_segment.

**[I-8] compliance.** `cemented_bundle_aggregate(W_start - 2)` в candidate_ssha_init — canonical & unpredictable-offline компонент (зависит от ML-DSA-65 подписей confirmers окна W_start - 2). Атакующий с SSHA hardware advantage не может pre-compute init для произвольного будущего W_start — aggregate доступен только после cementing W_start - 2. Pre-computation grinding закрыт по построению.
...
1. **SSHA-барьер:** τ₂ окон последовательного хэширования (при нормальной нагрузке). Физическая работа, sequential по построению — не ускоряется параллелизмом.

2. **Adaptive SSHA:** стоимость кандидатуры пропорциональна давлению на сеть в момент **публикации** NodeRegistration (не в момент начала SSHA работы). Это закрывает timing-manipulation: attacker не знает заранее какое pressure будет в момент W_p.
...
if candidate_pressure(W) > 0.01:
    ...
    required_ssha_length(W) = τ₂_windows   (base_ssha_length)
```

| Ситуация | pending | active | pressure | required_ssha |
|----------|---------|--------|----------|--------------|
...
**Привязка к W_p (не W_start).** `required_ssha_length` вычисляется из canonical state **в момент cementing NodeRegistration (W_p)**. Кандидат декларирует `ssha_chain_length` в NodeRegistration — длину своей SSHA цепочки. Валидатор проверяет `ssha_chain_length ≥ required_ssha_length(W_p)` и корректность proof_endpoint через пересчёт SSHA от init на `ssha_chain_length` окон.
...
**Extension rule для honest operators.** Если первая попытка NodeRegistration rejected по `ssha_chain_length < required`, оператор может:
1. Продолжить SSHA работу от текущего proof_endpoint на дополнительные окна
2. Обновить NodeRegistration: новый proof_endpoint = SSHA(old_proof_endpoint, additional_length), ssha_chain_length = old + additional
3. Повторить publication с updated proof

SSHA работа не теряется — только admission откладывается. Honest strategy: consecutive SSHA extension пока required не удовлетворено.

**Self-correcting механика.** Чем сильнее давление → тем длиннее required SSHA → дороже Sybil → давление падает через admission или expiry. При снижении давления (expiry 3τ₂ для просроченных кандидатов) → pending уменьшается → required нормализуется → легитимный вход восстанавливается.

**[I-8] compliance — grinding resistance.** Attacker не может предсказать `required_ssha_length(W_p)` в момент начала SSHA: pressure зависит от будущих cemented NodeRegistrations и будущих BCs (active_nodes). Attacker не контролирует privкey honest participants → не может предвычислить pressure. Forced over-provisioning или extension rule.

**Timing manipulation закрыта.** Attacker не может начать SSHA при низком давлении и подать при высоком — required проверяется на момент публикации. Minimum SSHA (base_ssha_length = τ₂) достаточен только если pressure(W_p) ≤ 1%. Иначе нужен extended SSHA пропорционально pressure.
...
Sybil-барьер для operator-аккаунтов: sequential SSHA `ssha_chain_length × D` SHA-256 хэшей до публикации NodeRegistration + selection_interval (336 окон) между событиями admission. Денежного барьера нет.
...
Каждый кандидат проходит τ₂ окон SSHA. Первые кандидаты появляются через τ₂ окон после genesis.
...
Compound-рост при постоянном entry rate: удвоение сети ≈ `1.5 × τ₂` после первой волны (детальная derivation — таблица «Обоснование протокольных констант → admission_divisor»). Первая волна лагает на τ₂ (SSHA вычисление первых кандидатов).

#### Barrier scope: что именно ограничено entry rate

Entry rate (τ₂ SSHA + selection event) ограничивает **только** участие узла в консенсусе. Операционная функциональность узла не зависит от его статуса в Node Table.
...
**Ограничено до entry в Node Table (после τ₂ SSHA + selection event):**
...
**Резюме:** барьер τ₂ SSHA защищает weight distribution и консенсусную безопасность. Он не ограничивает пользовательский доступ, пропускную способность сети, работоспособность новых узлов как инфраструктуры владельца, или скорость распространения сети среди пользователей.
...
- Confirmers (~100) публикуют BundledConfirmation (операции текущего окна + SSHA_Reveals предыдущего окна)
- Кандидаты (~12) публикуют SSHA_Reveal с lottery endpoint = SHA-256(T_r || node_id || window_index); reveals цементируются через BundledConfirmation следующего окна
- Лотерея (single-class, только узлы): winner = argmin(weighted_ticket_node) среди cemented SSHA_Reveal узлов-кандидатов (формулы `ticket_node`/`weighted_ticket_node` — integer spec per [I-9] в разделе «Класс 1: узлы» + «Integer log algorithm»). Аккаунты в лотерее не участвуют — см. «Аккаунты не участвуют в лотерее».
- Winner_{W-1} определяется proposer_W (= winner_{W-2}) из cemented SSHA_Reveals окна W-1 и cemented account operations окна W-1
...
TimeChain safety: компрометация значения TimeChain требует нарушения свойства последовательности SHA-256 SSHA.
...
**TimeChain** — глобальные часы. Чистая SSHA-цепочка `T_r = SHA-256^D(T_{r-1})`. Первичный продукт протокола. Источник времени и случайности. Продвигается по расписанию окон.
...
Лотерея single-class: участвуют только узлы через SSHA_Reveal с каноническим endpoint. Каждый узел производит weighted ticket по длине своей цепочки (`chain_length_snapshot`). Lowest weighted_ticket побеждает.
...
Победитель определяется после закрытия окна τ₁. Lowest `weighted_ticket_node` среди cemented SSHA_Reveal узлов-кандидатов = победитель. Единственный класс победителя — узел.
...
- **Остановка TimeChain** исключена: каждый узел вычисляет SSHA независимо
- **Искажение TimeChain** исключено: SSHA последователен, результат детерминирован
...
- **Sybil-барьер**: τ₂ окон SSHA + selection event (max 1% active_nodes за 336 окон) + weighted_ticket в лотерее
...
Лотерея single-class: конкурируют только узлы. Победитель — узел с lowest `weighted_ticket_node` среди cemented SSHA_Reveal кандидатов окна. `chain_length_snapshot` и `seniority_term` определяют вес — время и непрерывность работы единственный арбитр.
...
Consensus (протокольный слой):
  ...
  Candidate Pool             (node_id, pubkey, operator, proof_endpoint, W_start, ssha_chain_length, expires)
                             + persistent sparse Merkle tree (candidate_root)
  Proposals                  (навсегда)
  TimeChain SSHA + валидация   (1 ядро, 24/7, validation interleaved)
...
**Полнота сериализации snapshot.** Snapshot обязан содержать canonical byte-for-byte сериализацию всех записей каждой таблицы согласно определениям раздела «Состояние сети» — ВСЕ поля каждой записи, включая производные (chain_length_snapshot, checkpoints), счётчики (last_confirmation_window, op_height, account_chain_length), SSHA-метаданные (ssha_chain_length, W_start, proof_endpoint) и pubkey material. Пропуск или изменение любого поля одной записи меняет её canonical serialization → меняется хэш листа Merkle tree → несовпадение с proposer-recorded root окна W → snapshot rejected, retry с другого пира.
...
Лотерея Монтана single-class: эмиссию получают только узлы (через SSHA_Reveal с каноническим endpoint). Пользовательская активность не даёт прямой эмиссии — она создаёт спрос на инфраструктуру узлов через `Transfer` оплаты прикладным сервисам (мессенджер, премиум, хранение, подписки), которые сами поднимают узлы и попадают в lottery operator pool. Путь Лестницы суверенности (Account → Operator) — единственный protocol-level способ начать зарабатывать эмиссию — см. раздел «Эмиссия → Двигатель роста сети» и «Два пути участия».
...
Верификация любым третьим лицом, без доверия узлу Монтаны:
  ...
  7. Пересчитать TimeChain SSHA от proposal окна W до genesis по prev_proposal_hash
```
...
Минимальный набор примитивов: `Transfer` (перевод существующему), `TransferActivation` (создание AccountRecord), `Anchor`, `ChangeKey`, `CloseAccount` — всё что нужно для канонического порядка событий и переводов ценности. Sequential time (SSHA iteration count, τ-окна, chain_length) — единственный consensus-critical cost.
...
6. Проверить timechain_value(W) пересчётом TimeChain SSHA от T_{W-1}
7. Проверить цепочку proposals от W до genesis (prev_proposal_hash)
```

Шаги 1, 2, 3a, 3b, 5: O(1) операций. Шаг 6: `D` хэшей на одном ядре (один сегмент TimeChain SSHA). Шаг 7: линейная проверка подписей и хэшей по цепочке proposals от окна W до genesis.

Полная верификация от генезиса: H сегментов TimeChain SSHA, каждый независим. На C ядрах: ~(H/C) × D хэшей. TimeChain хранит все промежуточные T_r в proposals — параллелизация полная.
...
| NodeRegistrations в incremental apply W_p | nr_sort_key(nr) ascending | формула раздела Adaptive SSHA |
...
| `mt-ssha-reveal` | Class domain для identifier(reveal) SSHA_Reveal — Правило R2 |
| `mt-account` | Деривация account_id = SHA-256("mt-account" \|\| suite_id \|\| pubkey) |
| `mt-node` | Деривация node_id = SHA-256("mt-node" \|\| node_pubkey) |
| `mt-candidate-ssha-init` | SSHA init seed для кандидата на вход в сеть |
...
| `mt-timechain` | TimeChain SSHA seed |
...
| SSHA scheduling | Управление TimeChain SSHA |
...
- **Lottery конституция:** chain_length-weighted formula с seniority_term; time-as-resource — единственный неприобретаемый ресурс веса; committee selection через SSHA + sortition; canonical winner selection через cemented SSHA_Reveals
- **Open financial layer ([I-2]):** балансы, суммы переводов, отправители, получатели — публичны на уровне протокола
- **Time-based scarcity model ([I-15]):** anti-spam, anti-bloat и Sybil защиты через канонические time-based примитивы (rate-per-identity, TTL через активность, chain_length thresholds, sequential SSHA iteration count, cooldown активации, [I-8] cemented_bundle_aggregate binding)
- **Pay-by-time, not by-money:** единственная operation cost — sequential time (SSHA iteration count для NodeRegistration, τ-окна для cooldown, chain_length для seniority); экономические потоки между аккаунтами выражаются через `Transfer`, не через protocol-level operation cost
...
|-----------------|----------------------------|---------------------|
| Криптографические примитивы (подписи, SSHA, hash) | 2⁻¹²⁸ (полная криптографическая стойкость) | Стандарт криптографии; lattice-based примитивы ML-DSA-65 и ML-KEM спроектированы на этом уровне |
...
|-----------|----------|-------------|
| τ₁ (одно окно) | `D` хэшей TimeChain (Genesis: D₀ = 325 000 000) | Class: Operational/Performance. Окно — нормативно `D` последовательных SHA-256 итераций (не физические секунды per [I-18]). Genesis hardware target: ≈ 60 кварцевых секунд на эталоне (iMac M1 2021 idle, 5.097 MH/s × 60 s + runtime correction). UX bound rationale: confirmation within ≈1 min subjective threshold [Nielsen 1993 Usability Engineering]. SSHA lower bound: τ₁ существенно превышает typical gossip propagation. Network diameter при 24 outbound connections: log_24(N) hops; для N = 10⁵ nodes = log_24(10⁵) ≈ 3.6 hops × 300ms single-hop latency ≈ 1.1 s. Safety factor ×20 для worst-case variance: τ₁ ≥ 22 emergent seconds [Boneh et al. 2018 CRYPTO «Verifiable Delay Functions» — SSHA timing requirements]. Band [22, 60] emergent на genesis-железе. Pin при D₀ = 325 000 000 даёт верхнюю границу диапазона на genesis hardware, maximizing SSHA work within UX budget для maximum hardware-asymmetry margin |
...
| D₀ (TimeChain SSHA за окно) | 3.25 × 10⁸ (= 325 000 000, hex 0x135F1B40 — authoritative SSOT в Указе Генезиса → «Калибровка D₀» per [I-10]) | Class: Cryptographic/Performance. Единственный исторический quartz-замер на genesis-железе (iMac M1 2021 idle, single-thread): median SHA-256 rate 5.097280 MH/s × 60 кварцевых секунд = 305 836 793 хэшей; runtime-corrected × (60 / 56.35) = 325 000 000 учитывая SSHA interleaving с consensus работой. Полная derivation methodology — Указ Генезиса. **Режим: sequential single-chain SSHA.** Hardware advantage через pipelined single-thread оптимизацию ограничен ×5-10 над commodity [Pietrzak 2018 «Simple Verifiable Delay Functions», Boneh et al. 2018 CRYPTO «Verifiable Delay Functions»]. Монтана использует exclusively sequential regime: каждая итерация SHA-256 зависит от предыдущей, параллелизация архитектурно исключена |
| base_ssha_length (SSHA entry) | τ₂ (20 160 окон) | Class: Sybil resistance (combined defense). **Component** барьера: sequential SSHA cost + AS diversity filter. SSHA cost: 1 τ₂ canonical работы commodity / ≈ τ₂/10 на ASIC×10 — emergent ~$20-50 per candidate rent (illustrative, market dependent). AS diversity filter: attacker bounded by actually controlled AS count (typical large attacker controls 10-100 AS из global pool ~80 000). Combined defense multiplier: для 1000 Sybil candidates attacker spends $20-50k SSHA rent AND должен распределить по minimum 150 distinct AS (per committee_divisor L1 requirement); combined barrier = SSHA cost × (required AS count / attacker AS capacity) ≈ 10-100× stronger чем SSHA alone. Unit consistency = τ₂ (1 adaptation epoch = 1 entry epoch) |
...
| Ядра на узел | минимум 1 | Class: Operational. TimeChain SSHA sequential — выполняется на одном ядре последовательно. 1 ядро достаточно, validation interleaved с SSHA. 2+ ядра устраняют interleaving overhead (~5-10%) |
...
| adaptive_ssha_threshold | 0.5% (pending/active) | Class: Adaptive. Stationary pending ratio = 1/D_adm = 1/256 ≈ 0.39%. Buffer factor β = 1.28 [standard control-systems 20-30% hysteresis]. **P_thr = β × 0.39% = 0.5%** |
| adaptive_ssha_multiplier | ×200 | Class: Adaptive. **Math continuity**: required_ssha = base × pressure × M. At pressure = P_thr = 0.005, required = base ⟹ **M = 1/P_thr = 200**. Derivation follows from continuity requirement |
| base_ssha_length | τ₂ окон (= 20 160) | Class: Sybil resistance. См. «Криптографические и временные параметры» выше (combined defense articulation) |
| max_ssha_horizon | 4 × τ₂_windows (80 640) | Class: Security (adaptive SSHA upper bound). В BFT-контексте с 33% Byzantine tolerance покрывает pressure до ρ_max = 2% (4× P_thr) для spam/surge defense. Social consensus coordination handles beyond-BFT scenarios. H_max = B × ρ_max × M = τ₂ × 0.02 × 200 = **4τ₂** |
