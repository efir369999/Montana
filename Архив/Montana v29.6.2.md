...
|-----------|----------|-------------|
| τ₁ (длительность окна) | 60 секунд | Class: Operational/Performance. UX bound: confirmation within 1 min subjective threshold [Nielsen 1993 Usability Engineering]. VDF bound: τ₁ ≥ 10 × gossip_RTT ≈ 25s для VDF >> network latency [Boneh et al. 2018 CRYPTO «Verifiable Delay Functions»]. Band [25, 60]. Pin = 60s (upper edge maximizes VDF work within UX budget) |
| τ₂ (epoch boundary) | 20 160 окон (14 дней) | Class: Operational. Wall-clock target 14 days matches Bitcoin retarget period [Nakamoto 2008] — 15+ лет L1 production convention. τ₂_windows = 14 × 86400 / 60 = 20 160. Factorization 2⁶ × 3² × 5 × 7 (60 divisors) enables flexible sub-epoch division. Design choice с historical precedent |
| D₀ (TimeChain VDF за окно) | 252 × 10⁶ | Class: Cryptographic/Performance. Эмпирическая калибровка: median SHA-256 rate 4.2 MH/s на commodity x86_64 single-thread × 60s = 252 × 10⁶. **Режим: sequential single-chain VDF.** Hardware advantage atomic через pipelined single-thread оптимизацию ограничен ×5-10 над commodity [Pietrzak 2018 «Simple VDF», Boneh et al. 2018, Chia Network VDF papers]. Montana использует exclusively sequential regime: каждая итерация SHA-256 зависит от предыдущей, параллелизация архитектурно исключена |
| base_vdf_length (VDF entry) | τ₂ (20 160 окон) | Class: Sybil resistance (combined defense). **Component** барьера: sequential VDF cost + AS diversity filter. VDF cost: 14 дней wall-clock commodity / 1.4 дня на ASIC×10 = ~$20-50 per candidate rent. AS diversity filter: attacker bounded by actually controlled AS count. Combined defense (VDF × AS diversity) orders-of-magnitude сильнее каждого компонента в отдельности. Unit consistency = τ₂ (1 adaptation epoch = 1 entry epoch) |
| R_BASELINE (baseline emission) | 13 Ɉ/окно forever | Class: Economic (derived from infrastructure cost support + inflation target). **Derivation**: (1) Target долгосрочная годовая инфляция 2% [Friedman 1969 k-percent rule, central bank convention]. (2) Target long-term supply ≈ 342M Ɉ (design choice decimal scale). (3) Math: R_BASELINE = 0.02 × 342 000 000 / 525 960 = 13.005 → integer pin = **13**. **Economic validation**: honest operator infrastructure cost $1 200-2 400/год (2 CPU cores dedicated server + internet + reliability). При target N_steady = 10 000 operators per-operator annual reward = 13 × 525 960 / 10 000 = 684 Ɉ/год — устанавливает Ɉ market value floor ≈ $2 для operational sustainability |
| BOOTSTRAP_EPOCHS (bootstrap length) | 5 | Class: Economic (bootstrap duration). **Derivation**: Target duration 5 лет based на empirical L1 maturation timeline. Bitcoin достиг ~10 000 nodes за 3-5 лет (2009-2013); Ethereum аналогично (2015-2019); «decentralization achieved» metric для L1 около 5 лет. 1 epoch = 1 year (calendar alignment) → **BOOTSTRAP_EPOCHS = 5** |
...
| stem_epoch (Dandelion++) | 10 окон | Class: Privacy. Dandelion++ paper [Fanti et al. 2018 SIGMETRICS «Dandelion++: Lightweight Cryptocurrency Networking with Formal Anonymity Guarantees» § 4] recommends stem epoch comparable к typical user transaction session duration. τ₁ = 60s, user session ≈ 5-30 min → 5-30 windows. Pin = 10 окон (middle, 10-min rotation) предотвращает multi-tx linking через same stem peer |
| Ядра на узел | минимум 1 | Class: Operational. TimeChain VDF sequential — не ускоряется параллелизмом. 1 ядро достаточно, validation interleaved с VDF. 2+ ядра устраняют interleaving overhead (~5-10%) |

### Безопасность консенсуса и сети

| Константа | Значение | Обоснование |
|-----------|----------|-------------|
| confirmation_quorum | 67% | Class: Cryptographic/BFT. Math необходимость: Byzantine fault tolerance n ≥ 3f+1, quorum 2f+1 = 2/3+1 [Castro & Liskov 1999 «Practical BFT»; Buchman 2018 Tendermint]. FLP impossibility [Fischer Lynch Paterson 1985 «Impossibility of Distributed Consensus with One Faulty Process»] устанавливает tight bound для async deterministic consensus. **Математическая necessity, derivation строгая** |
| committee_divisor (confirmation_threshold) | active_chain_length / 256 | Class: BFT security + implementation efficiency. Три независимых пинающих требования пересекаются в единственном значении 256: (L1) живучесть кворума при honest uptime 0.95 требует N ≥ 150; (L2) нагрузка связи при O(N²) агрегации подписей в бюджет 10 Mbps даёт N ≤ 500; (L3) степень двойки для bitmap-alignment, bitwise-routing, SIMD-обработки, balanced Merkle tree. Единственное значение в [150, 500] удовлетворяющее всем трём — **256**. Безопасность: при uptime asymmetry ≤ 1.18× и `f ≤ 0.25` в сети доля атакующего в комитете ≤ 28.2%, ниже BFT threshold 1/3. Требование к развёртыванию: операторы ≥ 0.85 онлайн-работы |
| admission_divisor (slots per selection) | max(1, active / 256) | Class: Admission capacity. **Fixed divisor**. Consistency с committee_divisor упрощает имплементацию. Adaptive behavior применяется только к VDF length через pressure. slot_min = 1 гарантирует network liveness при малом active count |
| outbound connections | 24 | Class: Network security (eclipse). Модель: attacker контролирует f = 0.3 peer-пула [Heilman et al. 2015 USENIX «Eclipse Attacks on Bitcoin», Marcus et al. 2018 «Low-Resource Eclipse Attacks on Ethereum»]. Target P(eclipse) < 2⁻⁴⁰ [TLS 1.3 RFC 8446 industry standard]. Math: P(eclipse) = f^N < 2⁻⁴⁰ ⟹ N > 40·log(2)/\|log(0.3)\| ≈ 23.03 ⟹ smallest integer **N = 24**. Bandwidth cost ~24 KB/s outbound — не binding. Diversity selector (≥7 distinct AS) снижает effective f, усиливая margin |
| equivocation timeout | 10 окон | Class: BFT detection. BFT evidence propagation [Castro & Liskov 1999]: cement evidence 3 окна + propagation buffer 2 окна + 2× safety factor = **10 окон**. Sufficient для worst-case gossip propagation без delayed slashing |
| active predicate | 2τ₂ (40 320 окон) | Class: Operational lifecycle. Один full epoch downtime (maintenance) + recovery buffer. 1τ₂ слишком агрессивно для operator weekly maintenance cycles (2 weeks); 3τ₂ — dead nodes bloat state |
...
| candidate_expiry | 3τ₂ (60 480 окон) | Class: Operational. Queuing analysis: at slot_rate ≈ 0.5% × 60 events/τ₂ × 3τ₂ = 180 events, P(not picked) = 0.995^180 ≈ 0.404, P(admitted) = **59.6%**. 2τ₂ даёт P(admitted)=45% (слишком низко); 4τ₂ даёт 70% (Pool bloat) |
...
| chain_length_snapshot | скользящее окно 6τ₂ (120 960 окон) | Class: Lottery weight (recency). Target: time для new honest operator достичь full snapshot parity с established = snapshot_window. 6τ₂ ≈ 84 дня — balance motivation (не слишком долго discourage) и lottery stability (не слишком быстро weight churn). 4τ₂ — faster parity; 8τ₂ — matches node_pruning но slower |
