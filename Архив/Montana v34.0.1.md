...
Обоснование: Sybil на голосование/лотерею закрывается chain_length-weighted механизмами (узлы) и activity-based pruning (аккаунты), но это не адресует resource consumption через fan-out. Миллион аккаунтов не меняет распределения лотерейных весов, но занимает ×миллион `AccountRecord` в state trie. Time-based cooldown активации `1 TransferActivation per τ₂` для аккаунтов и sequential VDF `vdf_chain_length × D` для кандидатур узлов вместе закрывают оба вектора через канонические time-based примитивы.

Conformance audit существующих persistent tables:

| Таблица                | Защитный путь                                  | Статус       |
|------------------------|------------------------------------------------|--------------|
| `AccountRecord`        | [I-14].2 activity-based: activation cooldown `1 TransferActivation per sender per τ₂` (через поле `last_activation_window`) + 1-op-per-τ₁ rate-limit + pruning (`balance == 0` + 4τ₂) | закрыто     |
...
**Storage Cards учитывают актуальные размеры подписи ML-DSA-65** (`account_pubkey 1952 B`, `signature 3309 B`, `secretkey 4032 B`) для всех persistent state tables. Защитный механизм для каждой таблицы — sequential time barrier (NodeRegistration VDF) либо activity-based pruning, не cost-per-byte бюджет.
...
Антиспам через время: право на операцию = доказанное время существования аккаунта (account_chain_length + last_activation_window).
...
Аккаунты: активируются sponsor-ом через `TransferActivation`. Рост пользовательской базы определяется распространением Монтана через цепочку спонсорств — каждый новый пользователь требует existing-аккаунт с положительным балансом, готовый передать первичный перевод и прошедший cooldown `1 TransferActivation per τ₂`. Самоинициация создания невозможна.
...
0. Боб offline: генерирует ML-DSA-65 keypair, вычисляет account_id
   ...
   (QR / сообщение).
1. Алиса (sponsor, баланс ≥ первичный amount, прошедший cooldown 1 TransferActivation per τ₂)
...
Только узлы. Победитель окна `W` получает `reward_moneta(W) = EMISSION_moneta = 13 Ɉ` на `operator_account_id`. Вес в лотерее = `chain_length_snapshot + seniority_bonus`. Связь с пользовательской базой узла — **не линейная**:
...
Сценарий: standalone оператор без app, N_active = 1000 узлов
  ...
  τ₁_windows ≈ 525 600 в год wall-clock at 60s/окно
...
Минимальный набор примитивов: `Transfer`, `TransferActivation`, `Anchor`, `ChangeKey`, `CloseAccount` — всё что нужно для канонического порядка событий и переводов ценности. Sequential time (VDF iteration count, τ-окна, chain_length) — единственный consensus-critical cost.
...
- **Lottery конституция:** chain_length-weighted formula с seniority_bonus; time-as-resource — единственный неприобретаемый ресурс веса; committee selection через VDF + sortition; canonical winner selection через cemented VDF_Reveals
...
| Эмиссия (`EMISSION_moneta`) | governance pin | Константа за окно. Не выводится из external benchmark (cost-per-operator зависит от Ɉ price discovery, который сам функция от network adoption); pin = 13 Ɉ совпадает с divisor в lottery seniority_bonus formula (structural reuse). См. Constants table «EMISSION_moneta» |
