...
- **Post-genesis admission flow без bypass.** N_SEED определяет ТОЛЬКО initial cohort window=0; все узлы, регистрирующиеся после genesis (`NodeRegistration` operation в окне >0), проходят полный `CandidateSsha → Registered → Active` путь через standard `selection_event` без исключений.
