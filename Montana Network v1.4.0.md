...
Hard-coded discovery-seed peers `(IP, node_id, pubkey)` ship with the client so a newcomer with no peers can find the mesh. If every seed IP is blocked at the country level — a new node cannot enter the network through that channel alone. Five independent discovery channels exist. One of the five is enough.
...
```
1. Извлечь discovery-seed peers из встроенного списка (захардкожено)
2. Выбрать 1-3 random discovery-seed peer, connect
...
9. После успешного IBT с реальным peer — disconnect от discovery seed (освобождая seed slots)
10. Maintaining: PeerListRequest каждые ~τ₂ окон для обновления таблицы "проверенных" peers
```

Discovery-seed exceptional:
- Ограничение: не более 3 одновременных подключений к discovery seed на узел
...
Объект:                     источник случайности для транспортного слоя
                            ...
                            в Dandelion++, mesh_session_nonce,
                            backoff jitter)
...
```
Таблица:                          peer_penalty_table (для protocol violation
                                  blacklist; см. spec «Retry policy»)
Operation создающая запись:       peer disconnected с reason 0x04
                                  (protocol violation) либо IBT fail
Платит creation cost:             none — penalty entry создаётся passively
                                  при IBT fail или protocol violation event
Размер записи (bytes):            ≈ 100 B per peer_id
                                  (32B node_id + 8B blacklist_until_window
                                  + 8B reason_code + 16B observed_event +
                                  indexing 36B)
Secondary resources per record:   minimal — flat table
Cost per record:                  n/a — реактивная запись
Lifecycle condition:              путь 2 (temporal) — penalty TTL
                                  от 1·τ₁ (IBT fail) до 24·τ₁
                                  (protocol violation reason 0x04)
Eviction policy:                  TTL-based; hard cap не критичен из-за
                                  peer-event rate
Total bytes hard cap:             при peak 1000 blacklisted peers
                                  × 100 B = 100 KB sustained
[I-14] путь:                      2 (temporal через TTL); rate-limit через
                                  event rate
Sabotage time-budget атаки:       attacker не может flood penalty table —
                                  она пишется реактивно на legitimate
                                  violation events; attacker контролирующий
                                  N peers может выдавать ≤ N protocol
                                  violations parallel, bound через
                                  Sybil-stake-cost узла регистрации
                                  (τ₂ окон sequential SHA-256)
...
Expanded coverage (additional boundary + edge per type to reach 47): остаётся
...
7. **Legal/regulatory pressure на discovery-seed peers** — hardcoded discovery-seed peers могут быть seized / forced offline государством. Mitigation: после первого подключения узел работает через discovered peers, discovery seeds нужны только при cold start или при потере всех peers; censorship-resistant discovery (deferred M6.5) — secondary path.
...
**Gate 0.5(d.2) Field name coverage** для четырёх новых полей `protocol_params`:
...
(IBT online/mesh, Uniform Framing, Transport Randomness, Peer
...
- 4 Genesis Decree fields (connection limits + timeout)
