...
> **Статус перевода:** этот mesh-paper является companion draft до внешнего аудита и не является публикационной версией для Metzdowd. Авторитетная исправленная базовая статья: [`Whitepaper Montana.md`](Whitepaper%20Montana.md). К нему применяются те же post-audit правки: sequential delay function над SHA-256 вместо VDF-терминологии, ограниченное post-quantum утверждение до миграции transport на Noise_PQ, и квалифицированный расчётом масштаб миллиарда аккаунтов.
...
Кроме того, у Bitcoin и его наследников остаются две неустранённые в их рамках уязвимости. Первая — все production-блокчейны строят защиту подписей на гипотезах сложности дискретного логарифма на эллиптической кривой. Алгоритм Шора [8] на достаточно большом квантовом компьютере ломает эти гипотезы за полиномиальное время. NIST в 2024 году стандартизировал постквантовую инкапсуляцию ключей и подписи (FIPS 203 [2], 204 [3]); крупные сети не мигрировали. Вторая — антиспам через комиссии плохо масштабируется при росте: при перегрузке мелкие операции вытесняются ценой, при недогрузке — спам возвращается на маржинальной стоимости. Layer-2 (state channels, rollups) переносит экономику, не устраняет дефицита.

Montana предлагает: цепь, consensus-подписи которой построены на постквантовых примитивах, и в которой антиспам работает на времени (а не на деньгах) [13]. Цепь продвигается sequential delay function над SHA-256, выдавая глобально упорядоченные окна по ≈60 секунд каждое; проверка стоит того же порядка работы, что и вычисление, поэтому это не VDF с быстрой верификацией в академическом смысле. Операции в окне ограничены тремя независимыми ресурсами времени: per-identity per-window, длиной цепи аккаунта и старшинством. Поверх этого слоя протокол развёртывает второй слой — mesh VPN, использующий Reality (xray) [14] для маскировки трафика под штатный TLS-handshake к легитимной публичной мишени.
...
Базовая цель — поддержка ≥10⁹ активных аккаунтов. Это количественная архитектурная цель, не завершённый benchmark claim: один только `AccountRecord` state даёт около 2.06 TB при 1B active, а snapshot fast-sync benchmarks остаются gate milestone M7.
