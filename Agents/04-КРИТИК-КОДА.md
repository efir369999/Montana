...
> Самодостаточный промпт. Прочитай `ВВЕДЕНИЕ.md`, `ГЛОССАРИЙ.md`, `Code/CRITIC.md` перед началом.
...
grep -rn "pub const" Code/crates/ | awk '{print $NF}' | sort | uniq -d

# Поиск дублирующих типов
grep -rn "pub struct" Code/crates/ | awk '{print $3}' | sort | uniq -d

# Поиск domain separators вне mt-codec
grep -rn "domain" Code/crates/ | grep -v "mt-codec" | grep -v "::domain::"
```
...
- `Code/CRITIC.md` — full critic role
- `Code/CLAUDE.md` v1.15.0 — architect role для понимания дизайн-решений
- `Code/docs/audit-checklist.md` — формат findings для external audit firm
- `External-Audit/` — примеры прошлых отчётов
