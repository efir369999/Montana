...
- **FN-DSA-512** (Falcon-512, выбран в финальном раунде NIST PQC selection, июль 2022; forthcoming FIPS 206; reference implementation production-ready) — подписи операций аккаунтов
...
Аккаунт создаётся явно через OpenAccount. Приглашений не требуется. Получатель обязан существовать в Account Table до получения перевода.
...
TimeCoin **открыт**: сумма эмиссии, winner_id, height — публичные поля proposal header. Это чеканка новых монет — она должна быть верифицируема каждым узлом без криптографических proofs.
...
```
1. Боб: кошелёк генерирует keypair → account_id (постоянный адрес)
...
3. Алиса формирует Transfer в своей цепочке:
   ...
   new_balance_commitment: Pedersen commitment нового баланса Алисы
   link_commitment:        Pedersen commitment суммы перевода
   balance_proof:          Bulletproof: new_balance >= 0, link_amount > 0,
                           old_balance = new_balance + link_amount
...
6. Каждый узел проверяет:
   ...
   Bulletproof валиден (суммы корректны без раскрытия значений)
   Pedersen commitments консистентны
...
8. 67% online chain_length подтвердили → финализирована (~0.3 секунды)
   balance_commitment Алисы: обновлён (new_balance_commitment)
   balance_commitment Боба: обновлён гомоморфно через link_commitment
   Кошелёк Боба: «финализирован»
```

### Баланс

Баланс аккаунта — Pedersen commitment в Account Table. Фактическое значение известно только владельцу (хранит blinding factor локально). Обновляется при финализации: Transfer отправителя заменяет balance_commitment на new_balance_commitment; входящий перевод обновляет balance_commitment получателя гомоморфно через link_commitment. Узлы верифицируют корректность через Bulletproof без раскрытия сумм.
...
4. Catch-up cemented операций после τ₂ checkpoint:
   ...
   - Запросить BundledConfirmations от пиров, проверить что суммарный chain_length подтвердивших узлов > 67% online_chain_length
...
| FN-DSA-512 | NIST PQC selection финал (июль 2022), forthcoming FIPS 206, reference implementation production-ready | Подписи блоков аккаунтов и proposals |
| Pedersen commitments | Ristretto group | Скрытие балансов и сумм |
...
│  blinding factors      │         │  T_r = SHA-256^D(T_{r-1})            │
│  локальная история     │         │  глобальные часы, источник           │
│  операций              │         │  случайности                         │
│                        │         │        │                             │
│  AccountChain          │         │        ▼                             │
│  (счётчик окон         │         │  NodeChain (per node)                │
│   активности)          │         │  S_{i,w} = SHA-256(S_{i,w-1} || T_w  │
│                        │         │            || node_id)               │
└──────────┬─────────────┘         │  доказательство присутствия          │
           │  операции             │  chain_length = окна с BundledConf.  │
           │  (FN-DSA-512 +        │        │                             │
           │   Bulletproofs)       │        ▼                             │
           └──────────────────────▶│  AccountTable                        │
                confirmations      │  balance_commitment (Pedersen)       │
