...
Conformance status: conformance pending v29.8.1 (test vectors прилагаются после reference implementation mt-lottery Phase C).
```
...
Conformance status: conformance pending v29.8.1.
```
...
Input:
  ...
  τ₂_windows: u64            (константа из ProtocolParams = 13000)
...
Conformance status: conformance pending v29.8.1 (test vectors после reference implementation Phase C).
```
...
Semantics: возвращает -ln(endpoint/2^256) × 2^64, округлённый toward zero.
  ...
  Максимум: endpoint = 0 клипируется до saturated u128::MAX (SHA collision probability negligible).

Algorithm (byte-exact):
  1. e_u256 = big-endian interpretation of endpoint
  2. если e_u256 == 0: return u128::MAX
  3. msb_position = 255 - leading_zeros(e_u256)           # in [0, 255]
  4. int_part_u64 = (255 - msb_position) as u64           # integer part of log2(2^256/e)
  5. # Normalize mantissa в [2^127, 2^128):
     mantissa_u128 = (e_u256 << (127 - msb_position)) low 128 bits
     # Note: при msb_position < 127 — left shift, при msb_position == 127 — identity,
     #       при msb_position > 127 — right shift abs(127 - msb_position).
  6. # Q64 fractional part ∈ [0, 1):
     x_q64 = ((mantissa_u128 - (1u128 << 127)) >> 63) as u64
  7. # log2(1 + x/2^64) approximation — degree-3 polynomial в Q64 fixed-point.
     # Coefficients C0, C1, C2, C3 фиксируются в v29.8.1 после reference implementation
     # проходит conformance против double-precision f64 reference в диапазоне x ∈ [0, 2^64).
     # Maximum error target: 2^-56 (ULP в Q64.64 результате).
     frac_q64 = poly3(x_q64, C0, C1, C2, C3) as u64         # ≈ log2(1+x/2^64) × 2^64
  8. log2_q64_u128 = ((int_part_u64 as u128) << 64) - (frac_q64 as u128)
     # -ln(x/2^256) = log2(2^256/x) × ln(2) = log2_q64 × ln(2)
  9. LN2_Q64 = 0xB17217F7D1CF79AB     # ln(2) × 2^64, truncated
     # Binding constant (same в Rust и Go). Derivation: ln(2) = 0.693147180559945...
     # × 2^64 = 12786308645202655659.625... → floor to 12786308645202655659 = 0xB17217F7D1CF79AB.
  ticket_q64_128 = (log2_q64_u128 * (LN2_Q64 as u128)) >> 64
     # u128 × u128 → u256 intermediate; верхние 128 бит отбрасываются через >> 64 и cast.
  10. return ticket_q64_128

Conformance: coefficients C0..C3 + exhaustive boundary test vectors — v29.8.1.
...
- Unsigned arithmetic по всей цепочке (требование [I-9])
- Bounded error: Δ(ln_q64(e), 2^64 × (-ln(e/2^256))) ≤ 2^-56 при x ∈ (0, 2^256)
```
...
Test vectors — `conformance pending v29.9.1` (3 binding test vectors для Algorithm M-1 и 3 для per-role derivation фиксируются после прогона reference implementation `mt-mnemonic`).
