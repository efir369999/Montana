...
Merkle-дерево глобального состояния. Три подкорня обновляются инкрементально на каждое state transition:
...
Все три root всегда соответствуют live state — никаких frozen snapshots.
...
4. Catch-up после окна W до текущего:
   - Запросить cemented UserObjects от пиров. Для каждой: проверить подпись FN-DSA-512, sender.frontier_hash == prev_hash (против settled state), balance >= amount (для Transfer)
   - Запросить cemented ControlObjects (NodeInvitation, NodeRegistration). Применить к Node Table и Invitation Table
   - Применить incremental update Merkle trees (account_root, node_root, invitation_root)
