...
Победитель окна W регистрирует одно окно канонического порядка: `reward_nɈ(W)` наноɈ. При финализации proposal окна W+1 выплата применяется (one-window lag):
...
Genesis State (до первого окна, supply = 0):
  ...
  protocol_params (каноническая сериализация, little-endian, фиксированная длина полей):
    ...
    nickname_base_price_nj         (16B)  100_000_000 (базовая цена никнейма = 0.1 Ɉ)
    ...
    anchor_per_kb_nj               (16B)  100_000 (0.0001 Ɉ/KB сверх первого)
    anchor_free_kb                  (2B)  1 (первый KB Anchor без оплаты)
    creator_subscription_min_nj    (16B)  100_000_000 (0.1 Ɉ мин. месячная подписка)
...
**Стартовая цена для никнеймов разной длины** (базовая цена = `10^8` nɈ = 0.1 Ɉ):

| Никнейм | Length tier | Alphabet tier | starting_price_nj | В Ɉ |
|---------|-------------|----------------|--------------------|------|
...
| k (число τ₂ с начала) | Цена nɈ | Цена Ɉ |
|------------------------|----------|---------|
...
```
Before:
  sender.balance             = 1_000_000_000_000 nɈ (1000 Ɉ)
  sender.service_credits_nj  = 0
  supply_nj                  = S₀

PurchaseCredits(amount_nj = 100_000_000_000):   # 100 Ɉ

After:
  sender.balance             = 900_000_000_000 nɈ (900 Ɉ)
  sender.service_credits_nj  = 100_000_000_000 nɈ (100 Ɉ эквивалент)
...
anchor_per_kb_nj                        = 100_000           # 0.0001 Ɉ/KB
anchor_free_kb                          = 1                 # первый KB бесплатно
creator_subscription_minimum_monthly_nj = 100_000_000       # 0.1 Ɉ минимальная месячная подписка
```
...
Одно окно τ₁ регистрирует одну единицу канонического порядка с эмиссией `reward(W) = R_BASELINE + bonus(W)`, где `R_BASELINE = 13 Ɉ` — вечная константа, а `bonus(W)` — конечный bootstrap component.
...
Каждое окно τ₁ регистрирует одно каноническое окно канонического порядка. Baseline 13 Ɉ остаётся константой на весь горизонт существования протокола. Bootstrap bonus hard-capped на `BOOTSTRAP_CAP`.
...
Победитель окна τ₁ — всегда узел — регистрирует одно окно канонического порядка и получает `reward(W)` Монтана (зачисляется на `operator_account_id` узла). Одно правило. Baseline неизменен с генезиса, bootstrap конечен и капирован.
