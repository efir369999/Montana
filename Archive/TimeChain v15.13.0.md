...
**Версия:** 15.13.0 (2026-04-03)

## Определение

Montana — цифровой стандарт времени. Сеть независимых VDF-осцилляторов, поддерживающих единую верифицируемую временную шкалу с криптографическим доказательством каждого момента. Каждая зарегистрированная секунда = 1 Montana (Ɉ).
...
|-----|----------|-----------|
| StateBlock | Перевод средств | FN-DSA-512 подпись, баланс >= 0, prev_hash, link_amount > 0, fee >= min_fee. Если получатель не существует — создаётся автоматически |
...
apply_proposal(state, proposal) -> state':
  ...
  Шаг 2: применить user_set в порядке позиции получения в NodeChain победителя.
    StateBlock:          обновить баланс отправителя, кредитовать баланс получателя.
                         Если получатель не существует в Account Table — создать запись (balance = link_amount).
...
Аккаунты: без ограничений. Любой владелец Montana может создать аккаунт любому, переведя средства на новый адрес. Рост пользовательской базы не ограничен протоколом.
...
Монета: **Montana** (тикер: $MONT, символ: Ɉ).

1 секунда = 1 Montana = 1 $MONT = 1 Ɉ = 1 000 mɈ = 1 000 000 μɈ = 1 000 000 000 nɈ

60 секунд = 60 Montana. Одно окно τ₁ регистрирует 60 прошедших секунд = 60 Montana.
