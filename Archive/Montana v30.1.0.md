...
|------------------------|----------------------------------------------|--------------|
| `AccountRecord`        | требуется выбор (1 / 2 / комбо)              | pending v30.2.0 |
...
| Anchor records         | требуется выбор (1 — per-KB burn уже есть, проверить ratio) | pending v30.2.0 |
...
Записи со статусом `pending v30.2.0` требуют явного выбора механизма защиты автором протокола + добавления соответствующих констант в Genesis Decree + интеграции в `apply_proposal`. До закрытия записей — `AccountRecord` creation и Anchor persistence остаются идентифицированными векторами slow-bloat.
