...
Монтана — персональный пиринговый интернет на протоколе канонического порядка событий. Защищённое хранение данных, приватная связь и монеты времени — на узле пользователя.

Протокол Монтаны — фундамент персонального интернета. Сеть независимых узлов, каждый из которых ведёт собственные часы проверяемой задержкой — последовательностью вычислений, которую нельзя ускорить, и результат которой любой участник может пересчитать и проверить. Все узлы через консенсус собирают свои часы в единую каноническую последовательность событий. **Вес узла в этом консенсусе есть длительность его непрерывного присутствия в сети.** Каждое канонически зарегистрированное окно порождает постоянную награду в тринадцать монет времени плюс затухающую начальную надбавку, исчезающую за пять эпох.

**Время Монтаны** — реляционная структура, образованная последовательным хэшированием внутри часов проверяемой задержкой и канонической упорядоченностью, устанавливаемой консенсусом между узлами. Внутри этой структуры время в протоколе существует как последовательность канонических событий. Монтана — самодостаточная система отсчёта: каноническая последовательность событий, которую внешние системы могут наблюдать и использовать как систему отсчёта для своих нужд.

### Три проблемы доверия

Монтана решает три проблемы, каждая без участия третьей стороны:

- **Доверие ко времени.** Протокол производит каноническую последовательность событий без внешних источников. Решено протокольным слоем: часы проверяемой задержкой, консенсус, окна времени.
- **Доверие к хранению.** Данные пользователя хранятся на его узле. Протокол предоставляет фундамент: идентичность аккаунта, фиксация содержимого в виде хэша размером тридцать два байта с привязкой к окну на всю жизнь сети, стимул держать узел постоянно в сети (лотерея, монеты времени). Хранение, шифрование, индексация — клиентский слой.
...
**3. Управление вниманием.** Персональный интернет не максимизирует время пользователя в системе, а минимизирует его. Дал нужное — отпустил. Нет алгоритмической ленты, нет рекламы, нет метрик вовлечения, нет автоматического воспроизведения. Бизнес-модель Монтаны — эмиссия через узлы, не торговля вниманием. *Реализация: экономическая конструкция протокола устраняет стимул торговать вниманием — эмиссия через лотерею узлов, не реклама и не подписка (см. разделы о монете времени и о лотерее). Конкретные решения по интерфейсу — отсутствие алгоритмической ленты, формат уведомлений, политика автовоспроизведения, устройство чата и каналов — находятся в спецификации приложения Монтаны и здесь не описаны.*
...
- **Каноническое время** — согласованный всеми узлами порядок событий, производимый часами протокола; вес узла в сети есть длительность его непрерывного присутствия в этом порядке.
...
Всё за пределами этих трёх первоэлементов — хранение данных, коммуникация, агенты, индексация, интерфейсы — реализуется клиентским слоем поверх протокола. Протокол — часы, бухгалтерия и нотариат. Серверов нет — каждый узел сети равноправен, принадлежит своему оператору и работает на своём оборудовании.
...
**Проблема.** Существующие системы измерения времени измеряют физическое время через доверенную инфраструктуру — серверы синхронизации или спутниковые созвездия. Компрометация такого сервера или отключение спутника нарушает временную шкалу для всех зависимых систем. Использование таких систем в консенсусе протокола вносит зависимость от внешнего наблюдателя.

**Решение.** Реляционная временная структура — сеть независимых узлов, каждый ведёт собственные часы проверяемой задержкой, производя каноническую последовательность событий через собственную работу. Каждый узел вычисляет цепочку событий автономно через последовательное хэширование. Результат однозначен и может быть пересчитан и проверен любым участником из общих входных данных протокола.

**Свойства.** Время Монтаны обладает четырьмя свойствами:

- **Монотонность.** Номер окна строго возрастает. Часы проверяемой задержкой последовательны — каждый хэш зависит от предыдущего. Канонический порядок событий однозначен.
...
- **Проверяемость.** Любой может пересчитать часы и проверить каждое событие последовательности.
- **Независимость.** Каждый узел считает самостоятельно, опираясь только на общие входные данные протокола.

Монтана и внешние системы измерения времени — системы разных типов. Внешние системы измеряют физическое время через внешние источники. Монтана производит каноническую последовательность событий через собственную работу часов и консенсуса.
...
**Решение.** Доказательство времени — механизм консенсуса, в котором влияние узла определяется исключительно длительностью его непрерывного присутствия в сети, измеренной в подписанных временных окнах. Вес узла равен длине его цепи узла — количеству окон, в которых узел криптографически доказал своё присутствие.
...
**Проблема.** Денежная политика фиатных валют определяется решениями комитетов и непредсказуема. Дефляционные модели с фиксированным потолком предложения создают ожидание роста цены и подавляют использование как средства обмена.

**Решение.** Поокнная эмиссия — денежная политика из двух компонентов: вечная постоянная награда и конечная начальная надбавка, спадающая по расписанию. Каждое окно времени Монтаны порождает награду, равную сумме постоянной базы и начальной надбавки: `reward(W) = R_baseline + bonus(W)` единиц монеты времени. Постоянная база `R_baseline = 13 Ɉ` не меняется на всём горизонте; начальная надбавка `bonus(W)` затухает до нуля за конечное число эпох.
...
apply_proposal step 2 (TimeCoin emission, W = proposal.window_index, winner = winner_{W-1}):
  r = reward_nɈ(W - 1)
  # лотерея single-class, winner всегда узел;
...
Публичное (верифицируемо всеми):
  TimeCoin:     reward(W) = 13 + bonus(W) Ɉ за окно (см. «Поокнная эмиссия»)
...
**[I-14] compliance через [I-15].** Защита от раздутия state достигается time-based путём: cooldown `1 TransferActivation per sender per τ₂` (см. Инварианты TransferActivation) ограничивает rate создания новых AccountRecord, tree-expansion атакой на 1M записей требует ≥20 поколений × τ₂ (≈3.3 года), keepalive-атака через постоянную активность видна статистически и упирается в 1-op-per-τ₁ rate limit. Существующее pruning (`balance == 0` + 4τ₂) закрывает dormant bloat. Денежные барьеры (`MIN_BALANCE`, periodic burn) не используются per [I-15].
...
Proposal header:
  ...
  winner_class           1B    <- 1=Node (единственное valid значение текущей схемы).
...
- `winner_id` — получатель TimeCoin. Лотерея single-class: winner — всегда узел, выигравший лотерею окна. `reward(W-1)` зачисляется на `operator_account_id` узла-winner-а в apply_proposal step 2.
...
- `winner_class == 1` (единственное valid значение текущей схемы — Node; значения `2..=255` зарезервированы для будущих расширений и **reject**-аются текущей версией. Поле определено как implicit enum в layout; explicit inclusion в invariant-list обязательно для предотвращения dropped validation и panic в apply_emission на malformed byte)
...
apply_proposal(state, proposal) -> state':
  ...
  Шаг 2: применить TimeCoin победителя.
    ...
    Proposal header.winner_class == 1 гарантировано validate_header (единственное valid значение
      текущей схемы; прочие — reject до apply_emission).
    operator_account = Node Table[winner_id].operator_account_id
...
**Два proposal от одного proposer_node_id в одном окне**: оба отклоняются (equivocation), fallback к следующему узлу-кандидату. Если этот узел был winner, он теряет `reward(W)` (winner всегда узел, winner_class = Node — единственно валидное значение текущей схемы).
