...
**3. Управление вниманием.** Персональный интернет не максимизирует время пользователя в системе, а минимизирует его. Дал нужное — отпустил. Нет алгоритмической ленты, нет рекламы, нет engagement metrics, нет autoplay. Бизнес-модель Montana — эмиссия через узлы, не торговля вниманием. *Реализация: экономическая конструкция протокола устраняет стимул торговать вниманием — эмиссия через node lottery, не реклама и не подписка (см. разделы «TimeCoin» и «Лотерея»). Конкретные UX/UI решения — отсутствие алгоритмической ленты, формат уведомлений, политика autoplay, дизайн чата и каналов — находятся в Montana App spec и здесь не описаны.*
...
Montana формализует **Sovereignty Ladder** — двухшаговую экономическую модель:

**Шаг 0: Пользователь аккаунта (onboarding).** Имеет account keypair (в смартфоне, хардварный кошелёк, любой client). Подключается к узлу в сети — собственному или чужому — через IBT уровень 3. AccountRecord появляется в Account Table при первом incoming `TransferActivation` от sponsor; self-initiated creation не требуется. Использует сеть: Transfer, Anchor, NicknameBid, Premium Subscriptions, Credits. Все платные услуги идут в burn через [I-13]. Protocol-level earning отсутствует. Off-protocol earning возможен через creator economy (платные каналы, подписки) — см. Montana App spec. Барьер: смартфон + первый incoming transfer (любой объём ≥ MIN_ACCOUNT_BALANCE_NJ от существующего аккаунта).

**Шаг 1: Оператор узла (earning).** Держит свой узел 24/7 + operator_account привязанный к узлу (+ опционально другие personal accounts). Максимальная суверенность: данные на своём железе, полное участие в консенсусе, заработок через node lottery (`reward(W)` за выигранное окно, см. раздел «TimeCoin»). Барьер: commodity hardware (минимум 1 ядро), 24/7 uptime, bandwidth, NODE_REGISTRATION_STAKE.
...
- ✓ Все финансовые операции (Transfer, Anchor, NicknameBid, ChangeKey, CloseAccount)
- ✓ Данные приложения: Anchor хэш публичен, контент зашифрован ключом пользователя — хостящий узел не видит содержимого
- — Protocol-level emission отсутствует (reward идёт операторам узлов; см. [I-13] deflationary sink)
...
**Экономика hosting.** Оператор узла предоставляет инфраструктуру для AccountChain'ов своих пользователей. AccountRecord'ы реплицируются всей сетью (часть consensus state, не локальное хранилище оператора); оператор выступает gossip-посредником и bundled confirmer для операций размещённых пользователей. Incentive оператора: растущая hosted user base → больше cemented операций через его узлы → эти операции попадают в его BundledConfirmation → активный chain_length растёт → больше шансов выиграть node lottery → больше TimeCoin. Бизнес-модель приложения = эмиссия через узлы, не fee per user. Creator economy (платные каналы, премиум-подписки) — дополнительный off-protocol income layer для app builders.

Montana делает оба пути посильными: свой узел — commodity hardware (минимум 1 ядро), открытое ПО, окупаемость через лотерею. Account-only — любой смартфон, подключение через приложение. Решение где на этой шкале быть — за пользователем; Sovereignty Ladder формализует естественный переход от использования сети к её обслуживанию.
...
Покрывает: покупку никнеймов (`NicknameBid`), покупку кредитов для услуг (`PurchaseCredits`), Anchor-плату за контент свыше первого килобайта, подписки на premium-профиль, любые будущие платные protocol-level услуги. Единственный направленный поток TimeCoin:
...
Обоснование: константная baseline-эмиссия (13 TC/окно forever) без противотока burn ведёт к бесконечной инфляции. Burn через активность сети — противоток, стабилизирующий макроэкономику. Активная сеть → высокий burn → дефляция → ценность TC растёт → реальная доходность оператора растёт даже при константной номинальной эмиссии.
...
Conformance audit существующих persistent tables на момент v30.1.0:

| Таблица                | [I-14] путь                                  | Статус       |
|------------------------|----------------------------------------------|--------------|
| `AccountRecord`        | activation path определён через TransferActivation (sponsor создаёт запись); ongoing barrier (MIN_ACCOUNT_BALANCE_NJ + опциональный burn/rent) требует выбора | partial — pending v30.3.0 |
| `NicknameTable`        | 3 (quota: ≤1 nickname per account) + 1 (NicknameBid burn) | закрыто     |
| `AuctionTable`         | 2 temporal (auction expiry τ₂) + 1 (bid burn) | закрыто     |
| Anchor records         | требуется выбор (1 — per-KB burn уже есть, проверить ratio с учётом replication factor; либо pересмотр к ephemeral event) | pending v30.3.0 |
| `NodeTable`            | 1 (node stake deposit) + 2 (unstake / inactivity prune) | требуется audit |
| Candidate Pool         | 2 (expiry по истечении selection window)     | закрыто     |

Записи со статусом `pending v30.3.0` требуют явного выбора механизма защиты автором протокола + добавления соответствующих констант в Genesis Decree + интеграции в `apply_proposal`. Для `AccountRecord` в v30.2.0 закрыт activation path (через TransferActivation с binding receiver_pubkey), но ongoing lifecycle barrier (MIN_ACCOUNT_BALANCE_NJ threshold + выбор между cost-based burn / rent / combo) решается в Пункте 3 работы и фиксируется в v30.3.0. До полного закрытия — `AccountRecord` остаётся идентифицированным вектором slow-bloat с partially applied защитой.
...
Type byte `0x01` **не выделен** — OpenAccount удалён в v30.2.0, активация AccountRecord выполняется через `0x0A TransferActivation` от существующего аккаунта (sponsor). Самоинициация создания аккаунта невозможна; AccountRecord появляется в Account Table только при первом incoming TransferActivation.
...
- `payload.amount >= MIN_ACCOUNT_BALANCE_NJ` (достаточно для активации — параметр Genesis Decree; см. [I-14] audit)
- `Account Table[sender].balance >= payload.amount`
- После apply:
  - `Account Table[sender].balance -= payload.amount`
  - `Account Table[sender].frontier_hash = identifier(op)`
  - `Account Table[sender].balance` после apply `>= MIN_ACCOUNT_BALANCE_NJ` (иначе — reject `InsufficientSenderBalanceAfterApply`; исключение — если операция является CloseAccount, см. ниже)
  - `Account Table[payload.receiver] = new_record(balance = payload.amount, current_pubkey = payload.receiver_pubkey, suite_id = payload.suite_id, frontier_hash = 0x00...00, creation_window = current_window, ...)`
...
payload      N B   <- sender (32B)
                   ...
                   || bid_amount_nj (16B u128 LE, TimeCoin в нано-единицах)
...
**PurchaseCredits** — покупка кредитов для off-chain услуг с немедленным сжиганием TimeCoin:

```
type          1B   <- 0x08 PurchaseCredits
prev_hash    32B
payload      48B   <- sender (32B)
                   || amount_nj (16B u128 LE, TimeCoin для конвертации в кредиты)
signature  666B
Итого:     ~747 B
```
...
Кредиты non-refundable после начисления — нельзя конвертировать обратно в TimeCoin. Они тратятся только через `ConsumeCredits` при фактическом использовании услуги.
...
# supply_nj НЕ изменяется — TimeCoin уже сожжён в момент PurchaseCredits.
...
apply_proposal step 2 (TimeCoin emission, W = proposal.window_index, winner = winner_{W-1}):
  r = reward_nɈ(W - 1)
  # v30.2.0: лотерея single-class, winner всегда узел;
  # winner_class в layout сохранено для wire-format compatibility,
  # валидно только winner_class = Node (см. «Аккаунты не участвуют в лотерее»).
...
```
r = reward_nɈ(W)
# v30.2.0: single-class лотерея, winner — всегда узел.
operator_account = Node Table[winner_id].operator_account_id
Account Table[operator_account].balance += r
```
...
Удалить все записи Account Table где:
  balance == 0                                            <- нулевой баланс (v30.2.0 transitional;
                                                              TODO v30.3.0: заменить на
                                                              balance < MIN_ACCOUNT_BALANCE_NJ
                                                              после добавления константы
                                                              в Genesis Decree per Пункт 3)
...
**TODO v30.3.0 [I-14] compliance.** Условие `balance == 0` — transitional в v30.2.0. Порог `MIN_ACCOUNT_BALANCE_NJ` (Genesis Decree) ещё не формализован (решается в Пункте 3 работы: выбор между cost-based burn / rent / combo модель). По Gate 14 шаг 8 (existing cleanup audit против [I-14]) условие `balance == 0` соответствует старой ментальной модели «запись легитимна пока не пуста», что не закрывает slow-bloat attack. До замены `balance == 0` → `balance < MIN_ACCOUNT_BALANCE_NJ` — `AccountRecord` остаётся с `partial — pending v30.3.0` статусом в [I-14] audit table (см. выше).
...
В лотерее v30.2.0 участвует **только один класс** субъектов — узлы (через VDF_Reveal). Аккаунты в лотерее не участвуют (см. раздел «Аккаунты не участвуют в лотерее» ниже). Каждый узел производит ticket, взвешенный по длине своей NodeChain.
...
4. **Sovereignty Ladder clarity.** Двойная выплата (account + node) размывала primary incentive перехода в оператора; одна выплата (только node) делает upgrade единственным protocol-level earning шагом.

`operation_for_lottery`, `weighted_ticket_account`, `WINNER_CLASS_ACCOUNT`, DS-3 инвариант, domain separator `"mt-account-lottery"`, раздел «Валидация лотерейного билета аккаунта» — удалены в v30.2.0 (breaking change; pre-mainnet допустим). Поле `account_chain_length_snapshot` **сохранено** как anti-Sybil threshold в `NicknameBid`; только лотерейное применение удалено.

Экономическая модель v30.2.0 (numeric equilibrium burn ↔ emission) формализуется в разделе «Экономическая модель» (см. ниже) с conservative projection user growth / node growth / TC price sensitivity.
...
5. Proposer_W собирает BundledConfirmation-ы окна W-1 и cemented set:
   ...
   ```
6. Из included_reveals_{W-1} извлекаются все node endpoints (v30.2.0 — только node candidates; cemented account operations из included_bundles_{W-1} релевантны для apply_proposal по балансам/nickname/credits, но НЕ участвуют в лотерее).
...
| TransferActivation | Перевод с активацией нового AccountRecord | FN-DSA-512 подпись sender, prev_hash, receiver **не** существует, receiver == H("mt-account" \|\| suite_id \|\| receiver_pubkey), amount ≥ MIN_ACCOUNT_BALANCE_NJ, sender.balance >= amount |
...
- **included_bundles_{W-1}**: BundledConfirmation окна W-1 (суммарный chain_length ≥ 67% active_chain_length). Из included_bundles извлекаются cemented account operations для apply_proposal (баланс, nickname, credits, account_chain_length increment); account operations НЕ участвуют в лотерее в v30.2.0.
- **included_reveals_{W-1}**: VDF_Reveals окна W-1, cemented через BundledConfirmation окна W (67% active_chain_length). Из cemented reveals определяется `winner_{W-1}` (получатель `reward(W-1)` за окно W-1). В v30.2.0 лотерея single-class — winner всегда узел; cemented account operations окна W-1 не участвуют в выборе winner-а.
...
Proposal header:
  ...
  winner_class           1B    <- 1=Node (единственное valid значение в v30.2.0).
                                  2..=255 reserved for future protocol extensions; **reject** на текущей версии.
...
- `winner_id` — получатель TimeCoin. В v30.2.0 лотерея single-class: winner — всегда узел, выигравший лотерею окна. `reward(W-1)` зачисляется на `operator_account_id` узла-winner-а в apply_proposal step 2.
...
- `winner_class == 1` (единственное valid значение в v30.2.0 — Node; значения `2..=255` зарезервированы для будущих расширений и **reject**-аются текущей версией. Поле определено как implicit enum в layout; explicit inclusion в invariant-list обязательно для предотвращения dropped validation и panic в apply_emission на malformed byte)
...
TransferActivation: sender.balance -= amount, sender.frontier_hash = H(op)
              update_merkle_path(sender)
              создать запись Account Table[receiver] = { balance = amount, current_pubkey = payload.receiver_pubkey, suite_id = payload.suite_id, frontier_hash = 0x00...00, creation_window = current_window, ... }
  ...
  Шаг 2: применить TimeCoin победителя.
    r = reward_nɈ(current_window - 1)
        = R_BASELINE_nɈ + ((current_window - 1 < BOOTSTRAP_H × BOOTSTRAP_EPOCHS)
                           ...
                           : 0)
    Proposal header.winner_class == 1 гарантировано validate_header (единственное valid значение
      в v30.2.0; прочие — reject до apply_emission).
    operator_account = Node Table[winner_id].operator_account_id
  ...
  protocol_params (каноническая сериализация, little-endian, фиксированная длина полей):
    ...
    nickname_reserved_list          (160B)  lex-отсортированный список из 10 имён по 16B
                                             ...
                                             {admin, bootstrap, genesis, help, info,
                                              montana, root, support, system, timecoin}
    voice_call_rate_per_min_nj     (16B)  1_000_000 (0.001 TC/мин голосового звонка)
    video_call_rate_per_min_nj     (16B)  5_000_000 (0.005 TC/мин видеозвонка)
    premium_profile_monthly_nj     (16B)  10_000_000_000 (10 TC/мес премиум-профиля)
...
Аккаунт активируется спонсором. Получатель генерирует FN-DSA-512 keypair → вычисляет `account_id = SHA-256("mt-account" || suite_id || pubkey)` offline → делится `receiver_pubkey` / `account_id` со sponsor-ом (QR, сообщение, out-of-band). Sponsor (существующий аккаунт с положительным балансом) публикует `TransferActivation` → операция cemented → AccountRecord получателя появляется в Account Table при settle окна. Самоинициация создания аккаунта невозможна; рост пользовательской базы цепляется за распространение TimeCoin через цепочку спонсорств.
...
- Transfer — исходящие переводы TimeCoin существующим аккаунтам.
...
Аккаунты: активируются sponsor-ом через `TransferActivation`. Рост пользовательской базы определяется распространением TimeCoin через цепочку спонсорств — каждый новый пользователь требует existing-аккаунт с положительным балансом, готовый оплатить `account_creation_fee_nj` + первичный перевод. Самоинициация создания невозможна.
...
- Лотерея (v30.2.0 — single-class, только узлы): winner = argmin(weighted_ticket_node) среди cemented VDF_Reveal узлов-кандидатов (формулы `ticket_node`/`weighted_ticket_node` — integer spec per [I-9] в разделе «Класс 1: узлы» + «Integer log algorithm»). Аккаунты в лотерее не участвуют — см. «Аккаунты не участвуют в лотерее».
...
Лотерея в v30.2.0 — single-class: участвуют только узлы через VDF_Reveal с каноническим endpoint. Каждый узел производит weighted ticket по длине своей цепочки (`chain_length_snapshot`). Lowest weighted_ticket побеждает.
...
**Два proposal от одного proposer_node_id в одном окне**: оба отклоняются (equivocation), fallback к следующему узлу-кандидату. Если этот узел был winner, он теряет `reward(W)` (в v30.2.0 winner всегда узел, winner_class = Node единственно валидное значение).
...
3. Применить только первый; остальные отвергаются как «проигравшие этот раунд», TimeCoin не списывается (никакого escrow для проигравших в одном окне)
...
Дефолтный список в v30.0.0:
```
["montana", "admin", "bootstrap", "genesis", "timecoin",
...
```
voice_call_rate_per_min_nj              = 1_000_000         # 0.001 TC/мин
video_call_rate_per_min_nj              = 5_000_000         # 0.005 TC/мин
premium_profile_monthly_nj              = 10_000_000_000    # 10 TC/мес
...
`closed v30.0.0` — binding test vectors для `price_at` (8 пунктов), `starting_price_nj` (8 длин × 3 алфавитных тира = полный matrix), `PurchaseCredits` burn (2 вектора), `ConsumeCredits` списание (2 вектора). Полный набор в разделе «Binding test vectors» выше.
...
Победитель окна τ₁ — всегда узел — регистрирует одно окно Montana Time и получает `reward(W)` TimeCoin (зачисляется на `operator_account_id` узла). Одно правило. Baseline неизменен с генезиса, bootstrap конечен и капирован.

Лотерея v30.2.0 single-class: конкурируют только узлы. Победитель — узел с lowest `weighted_ticket_node` среди cemented VDF_Reveal кандидатов окна. chain_length и seniority bonus определяют вес — время и непрерывность работы единственный арбитр.
...
Sovereignty Ladder связывает активность пользователей и мотивацию запускать узлы:
...
Разработчики / пользователи поднимают узлы Montana → зарабатывают TimeCoin через node lottery
        ↓                                                         ↓
...
- Аккаунты не участвуют в лотерее — пользователь не получает эмиссию напрямую за активность. Путь к эмиссии — Sovereignty Ladder: поднять собственный узел (Operator роль) и зарабатывать через node lottery
...
Лотерея Montana v30.2.0 single-class: эмиссию получают только узлы (через VDF_Reveal с каноническим endpoint). Пользовательская активность не даёт прямой эмиссии — она питает сеть через deflationary sinks (PurchaseCredits, NicknameBid, Anchor fees) и создаёт спрос на инфраструктуру узлов. Путь Sovereignty Ladder (Account → Operator) — единственный protocol-level способ начать зарабатывать эмиссию — см. раздел «Эмиссия → Двигатель роста сети» и «Два пути участия».
...
= 1049 bytes (deterministic, fixed size; увеличено с 1000B в v29.x из-за никнейм-слоя в v30.0.0)
```
