...
**1. Агент-посредник.** ИИ-агент (Juno) действует строго от имени пользователя. Фильтрует и приоритизирует информацию по критериям владельца, не по алгоритмам платформы. Может ходить во внешний интернет, собирать данные, но решения о фильтрации принадлежат человеку. *Реализация: полностью клиентский слой. Полная спецификация Juno — sandbox-архитектура, permission levels, threat model, LLM runtime, signature delegation, audit trail — находится в Montana App spec. Protocol spec намеренно не содержит этих деталей: Juno — application-level механизм, протокол не знает о её существовании и не различает операцию подписанную вручную и операцию подписанную через Juno.*

**2. Локальное хранилище знаний.** Всё что пользователь читал, сохранял, получал — индексировано, доступно для поиска, хранится на его узле. Не на серверах корпорации. Контекст накапливается со временем — персональная база знаний. Протокол фиксирует факт существования (Anchor = 32B хэш). Содержание — на узле владельца, зашифровано его ключом. *Реализация: протокол даёт фундамент (Anchor, identity, ключевая инфраструктура) — это описано в данной спецификации. Клиентская часть — формат локального хранилища, шифрование контента ключом владельца, индексация, полнотекстовый поиск, структура базы знаний — находится в Montana App spec и здесь не описана.*

**3. Управление вниманием.** Персональный интернет не максимизирует время пользователя в системе, а минимизирует его. Дал нужное — отпустил. Нет алгоритмической ленты, нет рекламы, нет engagement metrics, нет autoplay. Бизнес-модель Montana — эмиссия через узлы, не торговля вниманием. *Реализация: экономическая конструкция протокола устраняет стимул торговать вниманием — эмиссия через node lottery и account lottery, не реклама и не подписка (см. разделы «TimeCoin» и «Лотерея»). Конкретные UX/UI решения — отсутствие алгоритмической ленты, формат уведомлений, политика autoplay, дизайн чата и каналов — находятся в Montana App spec и здесь не описаны.*

**4. Контроль данных.** Пользователь решает какие данные о нём существуют и кто имеет доступ. Не «политика конфиденциальности на 40 страниц», а технические механизмы: локальное шифрование на узле, выборочное предоставление доступа через адресное шифрование (ML-KEM), optional публикация профиля и контактов. Балансы публичны по дизайну ([I-2]). Всё остальное — решение владельца. *Реализация: протокол даёт инвариант [I-2] (открытость финансового слоя), Anchor (32-байтный хэш без содержания), identity и постквантовую ключевую инфраструктуру (ML-KEM) — это описано в данной спецификации. Клиентская часть — формат локального шифрования, протокол selective sharing, UI управления приватностью, формат публикации профиля и контактов — находится в Montana App spec и здесь не описана.*

### Клиентский интерфейс

Четыре слоя персонального интернета доступны массовому пользователю через клиентское приложение. Референс-реализация Montana App использует чат-центрированный интерфейс как наиболее доступную точку входа: переписка с контактами, платёжи к тем же контактам, личный контент и взаимодействие с агентом объединены в одной точке без переключения между приложениями. Конкретные UX-решения, устройство интерфейса и native platform integration описаны в Montana App spec.

Альтернативные клиенты — CLI, десктоп-приложения, специализированные интерфейсы для accessibility — допустимы и равноправны на уровне протокола. Выбор клиента не влияет на протокольные свойства аккаунта: seed, account_id и накопленный `account_chain_length` принадлежат пользователю, не конкретному клиенту (см. «Два пути участия» ниже).
...
**Путь роста.** Пользователь может начать как account-only, подключаясь через клиентское приложение (см. «Клиентский интерфейс» выше) — референсное Montana App использует чат-центрированный интерфейс, альтернативные клиенты допустимы. Позже — развернуть свой узел без потери AccountChain истории. account_id и все накопленные операции переносятся — ключ принадлежит пользователю, не узлу.
...
- **Единственное исключение** — inline commentary/intuition без binding claim: «примерно 13 TimeCoin baseline» в прозе как объяснение масштаба. Такие упоминания не normative и не создают дрifт когда authoritative value меняется, потому что явно маркированы как illustrative.
...
|---------------|-----------------|
| OpenAccount | `payload.suite_id` (новая identity) |
| Transfer, ChangeKey, Anchor | `AccountTable[sender].suite_id` |
...
Все операции — этот шаблон. `prev_hash` связывает операции в цепочку аккаунта. `signature` — FN-DSA-512 владельца над `signed_scope(op)` (см. Правило R1). `payload` зависит от типа. Все non-OpenAccount операции начинают payload с `sender (32B account_id)` — узел проверяет `Account Table[sender].frontier_hash == prev_hash` и `signature валиден для current_pubkey` за O(1).
...
**Инварианты OpenAccount:**

- `type == 0x01` (первый байт)
- `prev_hash == 0x00...00` (единственная операция аккаунта с нулевым prev_hash; иное значение — reject)
- `payload.suite_id` соответствует активной схеме подписи (на момент запуска: `0x0001` = FN-DSA-512); прочие — **reject** (UnsupportedSuite)
- `account_id = SHA-256("mt-account" || suite_id || pubkey)` уникален (нет в Account Table)
- Signature FN-DSA-512 valid для `payload.pubkey` над signed_scope (Правило R1; новая identity подписывает собственное создание)
...
- `payload.link` существует в Account Table (перевод на несуществующий account_id — reject)
...
Conformance status: closed v29.12.0 (binding test vectors выше).
```
...
```
Input:
  endpoint_account: [u8; 32]
  account_chain_length_snapshot: u32   (≥ 1 по DS-3)

Output:
  weighted_ticket_account: u128 (Q64.64, сравнивается с weighted_ticket_node)

Algorithm:
  ticket_q64_128 = ln_q64(endpoint_account)           # см. «Integer log algorithm»
  weighted_ticket_account = ticket_q64_128 / (account_chain_length_snapshot as u128)
    # u128 / u128 integer division toward zero

Совместимость: weighted_ticket_account и weighted_ticket_node — тот же Q64.64 тип, сравниваются через u128::cmp прямо в argmin.

Binding test vectors (byte-exact; все используют ln_q64 = 0x4f60bd6fe6504646 от TV3 endpoint):

  # A1 boundary DS-3 floor
  account_chain_length_snapshot = 1
  → weighted_ticket_account = 0x00000000000000004F60BD6FE6504646

  # A2 typical
  account_chain_length_snapshot = 100
  → weighted_ticket_account = 0x000000000000000000CB351828B400B3

  # A3 large
  account_chain_length_snapshot = 10000
  → weighted_ticket_account = 0x00000000000000000002083600683334

  # A4 u32 max (upper bound поля)
  account_chain_length_snapshot = 4_294_967_295
  → weighted_ticket_account = 0x0000000000000000000000004F60BD70
...
#### Integer log algorithm (per [I-9], shared между node и account lottery)

Общий алгоритм `ln_q64(endpoint) → u128` используется в обоих weighted_ticket формулах (узлы, аккаунты).
...
Conformance: closed v29.12.0 (binding coefficients + 5 test vectors выше).
...
- `winner_class ∈ {1, 2}` (1 = Node, 2 = Account per layout выше; прочие значения — **reject**. Поле определено как implicit enum в layout; отсутствие explicit inclusion в этот список ведёт к dropped validation и panic в apply_emission на malformed byte)
...
- Лотерея: winner = argmin(weighted_ticket) среди cemented VDF_Reveal nodes + cemented account operations (формулы `ticket`/`weighted_ticket` — integer spec per [I-9] в разделах «Класс 1: узлы», «Класс 2: аккаунты», «Integer log algorithm»)
...
Historical note: integer encoding изначально в v29.8.0 draft был Q32.32; 
после C4 re-audit кода mt-timechain (2026-04-19) перевыбран permille — 
...
**Аккаунты** участвуют в окне с финализированной cemented операцией. Каноническая формула `weighted_ticket_account` определена в разделе «Класс 2: аккаунты» выше. Весовой знаменатель — `account_chain_length_snapshot` (DS-3 invariant).
...
Формат `OpenAccount`, запись AccountRecord и функция `apply_proposal` в связи с данным разделом не изменяются. Мнемоника — локальный инструмент клиента; сеть видит только FN-DSA-512 pubkey аккаунта (и отдельно node_pubkey через `NodeRegistration`).
...
Все значения byte-exact, получены прогоном reference implementation `mt-mnemonic` (crates/mt-mnemonic в Таймчейн/Code/). Любая независимая реализация обязана воспроизводить идентичные hex-значения.
...
**Binding test vectors (domain-separated hash):**
  ...
  # Pre-v29.13.0 (без separator): BOTH preimage = "mt-app-encryption-key" → collision
  # v29.13.0 (с separator):
...
**Acceptable staleness bound.** Peer принимает `cached_window_index` в диапазоне `[peer.known_window_index - 7 × τ₁, peer.known_window_index]`. Свыше 7 × τ₁ (≈ 5 дней) cached значение признаётся слишком старым — peer отклоняет mesh IBT handshake и требует свежее значение через любой доступный канал до продолжения.
...
**Domain separator обязательно `mt-tunnel-mesh`, не `mt-tunnel`.** Отдельный separator критичен — иначе атакующий перехвативший online IBT proof (window slot = 2 окна, ≈ 8 секунд replay) мог бы использовать его в mesh контексте где staleness window расширен до 5 дней. Cross-context replay блокируется на уровне domain separation.

**Replay surface analysis.**
- Online IBT (separator `mt-tunnel`): replay window 2 окна ≈ 8 сек — узкий, acceptable.
- Mesh IBT (separator `mt-tunnel-mesh`): replay window расширен до 7 × τ₁ ≈ 5 дней, но replay блокируется per-nonce tracking.
...
| τ₂ (epoch boundary) | 20 160 окон (14 дней) | Class: Operational. Wall-clock target 14 дней = 2 calendar weeks aligns с operator weekly maintenance cycles. τ₂_windows = 14 × 86400 / 60 = 20 160. Factorization 2⁶ × 3² × 5 × 7 (60 divisors) enables flexible sub-epoch division. Design выбран для balance между responsiveness (шorter epochs = faster adaptation) и stability (longer epochs = reduced noise в participation_ratio measurements) |
...
| BOOTSTRAP_H (bootstrap epoch length) | 524 160 окон (26 × τ₂) | Class: Economic. **Derivation**: Target 1 год wall-clock per epoch. τ₂ = 14 дней. 365.25 / 14 = 26.09 → integer pin = 26 × τ₂. BOOTSTRAP_H = 26 × 20 160 = **524 160 окон**. Кратность τ₂ для совмещения с pruning / snapshot boundaries |
| BOOTSTRAP_R0 (bootstrap genesis emission) | 16 Ɉ/окно | Class: Economic. **Derivation**: Integer halving sequence over BOOTSTRAP_EPOCHS = 5, terminating at 1 (last epoch rate): r₁ → r₁/2 → r₁/4 → r₁/8 → r₁/16. Constraint r₁/16 = 1 (integer terminating value для preserving halving property) ⟹ **r₁ = 16 = 2⁴**. Sequence: 16 → 8 → 4 → 2 → 1 Ɉ/окно across 5 epochs. Power of 2 enables clean integer halving. **Validation incentive**: total emission каждой epoch (bootstrap + baseline) = {29, 21, 17, 15, 14} Ɉ/окно vs steady-state 13 Ɉ/окно. Каждая epoch содержит positive incentive above steady state (1 Ɉ additional minimum в epoch 5). Early epochs (≥2× steady state) обеспечивают strong incentive для initial network acquisition |
...
| R_BASELINE (baseline emission) | 13 Ɉ/окно forever | Class: Economic (derived from bootstrap dilution constraint). **Derivation**: (1) Design constraint — bootstrap pre-emission ≤ 5% supply в момент экономической зрелости. Обоснование порога: при pre-emission > 10% early participants получают disproportionate ownership, подавляя organic distribution; при < 2% bootstrap incentive слишком слаб для critical mass attraction. Порог 5% — centre sustainable band [2%, 10%], ограничивает dilution долгосрочных участников при сохранении attraction power bootstrap phase. (2) Экономическая зрелость = year 50 (protocol long-term horizon, generational alignment). (3) BOOTSTRAP_CAP = 16 248 960 Ɉ (derived выше). Math: S(Y=50) = BOOTSTRAP_CAP + R_BASELINE × T_year × 45. Constraint BOOTSTRAP_CAP / S(50) ≤ 0.05 ⟹ S(50) ≥ 324 979 200 Ɉ ⟹ R_BASELINE ≥ (324 979 200 − 16 248 960) / (525 960 × 45) = **13.04** → integer pin **R_BASELINE = 13**. **Verification инфляции**: R_BASELINE × T_year / S(50) = 13 × 525 960 / 323.94M = **2.11%** — соответствует Friedman 1969 k-percent rule central bank convention ~2%. **Пример operator economics (scenario, не design input)**: для случая N_active = 10 000 операторов per-operator annual reward = 13 × 525 960 / 10 000 = 684 Ɉ/год; фактические значения зависят от network adoption dynamics и market price discovery |
...
| active predicate | 2τ₂ (40 320 окон) | Class: Operational lifecycle. Один full epoch downtime (maintenance) + recovery buffer. 2 эпохи = 28 дней покрывает типичный operator maintenance cycle (weekly) с запасом. Значение sensitivity: 1τ₂ пересекается с weekly maintenance циклами; 3τ₂ удерживает inactive узлы в состоянии дольше необходимого |
| node pruning | 8τ₂ (161 280 окон) | Class: Operational lifecycle. 4× active_predicate (generous retry buffer). 112 дней inactivity practically permanent exit. 4τ₂ aggressive (может пропустить long-offline honest); 16τ₂ удваивает state bloat без benefit |
...
| account бакеты | 4^N × τ₂ | Class: Operational/Sybil. Exponential age stratification base 4. Sybil attacker isolated в Tier 0, получает 1/4 rate через round-robin. 4 tiers покрывают 0-256τ₂ (~10 лет) |
| D adjustment rate | ±3% за τ₂ | Class: Adaptive. Matched Moore's law pace: doubling time ln(2)/ln(1.03) × 14 days ≈ 329 days ≈ 0.9 years. Matches hardware generational improvements. ±1% слишком медленный response; ±10% волатильность, hardware churn |
...
| chain_length_snapshot | скользящее окно 6τ₂ (120 960 окон) | Class: Lottery weight (recency). Target: time для new honest operator достичь full snapshot parity с established = snapshot_window. 6τ₂ ≈ 84 дня (≈ quarter) — выбрано по принципу balance: window ≥ 2 × active_predicate (2τ₂) обеспечивает robust recency signal even при intermittent operator activity, window ≤ node_pruning (8τ₂) сохраняет consistency с lifecycle boundaries. Pin 6τ₂ — центр intersection [2τ₂ × 2 = 4τ₂, 8τ₂] = [4τ₂, 8τ₂], даёт 84 дня operator onboarding period. Value sensitivity: 4τ₂ ускоряет parity ценой lottery weight churn; 8τ₂ удлиняет onboarding ценой slower new operator integration |
| seniority_bonus divisor | 13 | Class: Lottery weight (longevity). Target T_cap = 3 года honest continuous operation до cap (infrastructure investment horizon convention). chain_length_at_cap = T_cap × T_year = 3 × 525960 = 1 577 880 окон. snapshot_max = 6τ₂ = 120 960. **Divisor = 1 577 880 / 120 960 = 13.04 ⟹ 13**. Math pin после target fixed |
| seniority_bonus formula | min(chain_length / 13, snapshot) | Bounded добавка за longevity с cap = snapshot (max advantage 2×). Через 3 года honest operator достигает cap, далее стабильный потолок |
