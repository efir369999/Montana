...
- **Контроль данных.** Пользователь решает что публиковать. Profile, encryption keys — всё optional. Данные на узле зашифрованы. Selective sharing через адресное шифрование (ML-KEM).
...
- Discovery: добавление контактов через QR-коды, invite links, direct account_id share; локальные псевдонимы (petnames) для контактов
...
| **discovery** | QR scanning, encryption pubkey lookup, local address book |
...
Session queue label не является app_id сам по себе. Canonical app_id формула протокола (Montana protocol spec, Anchor) требует `app_id = SHA-256("mt-app" || app_name)`. Queue label подставляется как `app_name`:
...
Пользователь делится `account_id` через QR-коды, invite links или direct share. Каждый контакт в локальной адресной книге получает **petname** — локальный псевдоним который пользователь задаёт сам, не опираясь на глобальные реестры.

### 7.1 QR code scanner и generator
...
### 7.3 Local address book + Petnames

Каждое приложение хранит свой локальный контакт-лист в encrypted SQLite.

**Принцип petnames.** В Montana identity — это `account_id` (32-байтовый хэш от pubkey). Этот идентификатор globally unique но для человека нечитаем. Чтобы работать с контактами удобно, пользователь присваивает каждому контакту **petname** — локальный псевдоним, видимый только ему. Никакая глобальная синхронизация petnames — это приватное имя в приватной адресной книге.

Petname независим от published profile контакта: контакт может называться в сети «Elena Petrova», но пользователь видит его локально как «Мама». Petname **приоритетнее** published display_name в UI.

**Contact entry:**
- `account_id` (32B, ключ — globally unique, derivation `SHA-256("mt-account" || 0x00 || suite_id || pubkey)`)
- `petname` (локальный псевдоним, задаётся пользователем при добавлении контакта; UTF-8 string до 64 символов; обязательное поле)
- `petname_set_at` (timestamp когда petname был assigned или обновлён)
- `trust_level` (added_via: `qr_scan` / `invite_link` / `direct_share` / `chat_reply`)
- `first_added_at` (timestamp первого добавления)
- `last_interaction` (timestamp последнего обмена сообщением / операции)
- `cached_published_name` (optional — последний display_name из contact's ProfileBlob; для справки)
- `cached_avatar_hash` (optional — последний avatar_hash из ProfileBlob; для справки)
- `notes` (optional — приватные заметки пользователя, видимые только ему)

**Petname flow:**
- При добавлении контакта через QR / invite / share — UI обязательно запрашивает petname **до** сохранения контакта («Как вы хотите назвать этот контакт?»). Pre-fill возможен из published `display_name` если контакт опубликовал ProfileBlob, но пользователь всегда может изменить.
- Petname изменяемый в любой момент через Contact Settings → Edit petname.
- Petname unique в пределах **локальной** address book пользователя (чтобы избежать confusion между двумя «Alice»). При collision — UI предлагает disambiguation («Alice (work)», «Alice (old phone)» и т.п.).
- При переходе между устройствами — petnames sync через зашифрованный backup blob на узле пользователя (если multi-device setup), но не публикуются никуда.

**Published profile vs petname:**
- Published profile: что контакт опубликовал о себе (через `ProfileBlob` в Application Layer, см. раздел 8).
- Petname: как пользователь видит этот контакт локально.
- Petname **всегда приоритетнее** published display_name для UI отображения.
- UI может показать published display_name рядом с petname мелким шрифтом («Мама · elena.petrova»), чтобы пользователь мог verify identity если контакт недавно изменил published profile.

**Защита от impersonation через petnames.**
- Petnames — local namespace, невозможно через них имитировать другого пользователя глобально (публично контакт виден только через `account_id`).
- При изменении published display_name контакта (detect через новый ProfileBlob Anchor) UI показывает мягкое уведомление: «Ваш контакт {petname} изменил публичное имя с «{old}» на «{new}». Petname остаётся неизменным.»
- Если два контакта в address book имеют одинаковый cached_published_name (например, оба «Alice»), petname differentiation forced при добавлении.

**Профиль контакта (cached):**
...
- Информация cache локально в `cached_published_name` / `cached_avatar_hash` и обновляется при новом Anchor в profile app_id от этого account.
- Cached fields используются только как вспомогательная информация (identity verification hint), не как primary display.
...
- Локальный поиск в synchronized `NicknameTable`
- При успехе — показ profile (имя, avatar из ProfileBlob если есть) и кнопка «Добавить в контакты»
- При неудаче — «Никнейм `@alice` не занят никем; попросите друга сообщить его account_id через QR, ссылку или mesh»

**UI предложения:**
- **Fuzzy search** опционально — если пользователь ввёл `@alyce`, предлагать близкие `@alice`, `@alyssa` из синхронизированной таблицы
- **Kana / Cyrillic input**: протокол использует только ASCII, клиент может показать keyboard layout hint

### 7.5 Nickname Auction UI

Приложение предоставляет полноценный интерфейс участия в аукционах:

**7.5.1 Browse available nicknames**

- Экран «Найти никнейм» с поиском по точному имени или по паттернам (`@*_photo`, `@a??`)
- Для каждого результата показывается статус:
  - **Свободен** (ещё никто не начал аукцион) — показывается `starting_price_nj` как entry price
  - **Голландская фаза** — показывается текущая `price_at(nickname, current_window)` + прогресс падения цены (сколько τ₂ осталось до пола)
  - **Английская фаза** — показывается текущий `highest_bid_nj` + время до `english_end_window` + число продлений
  - **Занят** — показывается владелец (`account_id` + петнейм если добавлен в контакты), кнопка «Занят, попробуйте другой»

**7.5.2 Place Bid flow**

1. Пользователь выбирает никнейм
2. Приложение проверяет eligibility локально:
...
3. Если не eligible — UI объясняет причину («Вам нужно быть активным в Montana ещё N дней» / «Вы уже владеете `@alice`, один никнейм на аккаунт» / «Недостаточно TC»)
4. Если eligible — показ confirmation:
   - Сумма в TC
...
**7.5.3 English phase monitoring**

- После перехода аукциона в английскую фазу (первая ставка) — уведомление пользователю: «Ваша ставка принята. Начался период перебид'ов длиной 2 τ₂. Вас могут перебить»
- Countdown до `english_end_window` в реальном времени
- Push-уведомление при перебиде: «Вас перебили на `@alice`. Текущая цена 8 TC. [Перебить +5%] [Пропустить]»
- Автоматический escrow: сумма ставки заблокирована на балансе, возвращается при перебид'е

**7.5.4 Acquisition completion**

- При истечении `english_end_window` и присуждении никнейма:
  - Push: «Поздравляем! Никнейм `@alice` теперь ваш навсегда»
  - Сумма сожжена из баланса (уведомление о burn: «5 TC навсегда изъяты из обращения»)
  - Никнейм появляется в Settings → My Nickname
  - Свой QR-код обновляется — теперь содержит никнейм для быстрого share

**7.5.5 My Nickname settings**

- Отображение текущего никнейма, даты покупки, уплаченной цены
- Кнопка «Показать proof of ownership» — для external sharing подтверждения владения (account_id + signature)
...
### 7.6 Sharing nickname

Пользователь может делиться никнеймом через любые существующие каналы (Signal, Telegram, email, SMS, устно):

```
«Я в Montana: @alice» 
→ получатель вводит @alice в свой Montana App 
→ локальный resolve в NicknameTable 
→ account_id получен 
...
Если получатель открыл ссылку на устройстве без синхронизированной `NicknameTable` — клиент сначала синхронизирует minimum subset таблицы (delta sync), потом резолвит.
...
## 19. Prepaid Credits (v3.0.0)

Слой UX для работы с кредитами на off-chain услуги. Протокол предоставляет три операции: `PurchaseCredits` (0x08) для сжигания TimeCoin и начисления кредитов, `ConsumeCredits` (0x09) для списания после использования услуги. Приложение скрывает эту сложность за простыми интерфейсами.
...
- Confirmation dialog: «Вы сжигаете N TimeCoin и получаете эквивалент кредитов. Сжигание необратимо. Кредиты нельзя вернуть в TimeCoin, только потратить на услуги»
- После подтверждения — публикация `PurchaseCredits` операции, ожидание cemented status
- Показ прогресса: «Операция в сети... cemented в окне W»

### 19.2 Credits Balance Display

- На главном экране — маленький widget с балансом кредитов + балансом TimeCoin
- Warning badge при `service_credits_nj < threshold` (по умолчанию порог — эквивалент 5 минут голосового звонка)
- Пуш-уведомление перед исчерпанием кредитов во время активного звонка: «Осталось 2 минуты звонка. Пополнить сейчас? [Да] [Нет]»

### 19.3 Auto-Top-Up (опционально)

- Настройка в Settings → Credits → Auto-Top-Up
- Пользователь задаёт:
  - Порог срабатывания (напр. «пополнять когда осталось меньше 5 TC кредитов»)
  - Сумма пополнения (напр. «покупать 50 TC кредитов за раз»)
- Приложение автоматически публикует `PurchaseCredits` при достижении порога
- Требует подтверждения биометрией / device password для защиты от непредвиденных списаний
- Автоматическое поведение логируется в local journal с полной историей

### 19.4 Usage History (локально)
...
- Export в CSV / JSON для персонального учёта
- Protocol-level запись — только `ConsumeCredits` в AccountChain (видна на-chain); детализированная история — клиент-side

### 19.5 Rate-таблица в приложении
...
| Anchor сверх 1 KB | 0.0001 TC/KB | — |
| Платная подписка на канал | 0.1 TC/мес минимум | устанавливает создатель |

Приложение переводит rates в привычные единицы при отображении.

---

## 20. Voice & Video Calls (v3.0.0)

Интеграция off-chain P2P аудио/видео-коммуникаций с protocol-level оплатой через prepaid credits. Технический стек — WebRTC или аналог; транспорт — mesh (mt-tunnel-mesh уже в протоколе) или прямое P2P через TimeChain relay.
...
- Проверка eligibility:
  ...
  - Если нет — предложить Top Up flow (см. раздел 19)
- Выбор качества видео: 360p (базовое) / 720p (стандартное) / 1080p (премиум, доступно не всем устройствам)
- Call request через messenger channel — собеседник принимает или отклоняет

### 20.2 Call connection

- P2P connection establishment:
  - Первичная попытка через mesh (если оба клиента в зоне mesh-обнаружения)
  - Fallback на TimeChain relay через узлы-операторы
  - Encryption derived from существующих ML-KEM-768 публичных ключей (в `EncryptionKeyBlob`)
  - Audio codec: Opus 24 kbps (базовое качество)
  - Video codec: VP9 или H.264 (зависит от device)
- ICE negotiation с fallback через multiple transports

### 20.3 Per-minute tracking

- Клиент локально отсчитывает минуты звонка
- Вычисляет `usage_nj = elapsed_minutes × voice_call_rate_per_min_nj` (или video rate)
- Каждые N минут (по умолчанию N=5) — публикует `ConsumeCredits` операцию для consensus-level settlement
- Если клиент закрывается / теряет сеть без publishing — накопленная usage теряется в favor пользователя (client hostile scenario защищён honest publishing behavior)

### 20.4 Call termination

- При завершении звонка (любой стороной / connection drop) — финальный `ConsumeCredits` за оставшуюся неотчитанную минуту
- Post-call screen: итоги (длительность, потрачено credits, качество звонка)
- Опциональный rating собеседника (только локально, для личной истории)
...
- Вся аудио/видео-связь идёт **прямо между устройствами**, не через protocol storage
- Metadata (кто кому звонил, когда, сколько минут) видна в `ConsumeCredits` операциях на-chain — это consensus cost
- Content звонка (аудио/видео byte stream) — E2E encrypted, никогда не записывается в Montana storage
- Пользователь может включить local recording (на своём устройстве) — но это клиент-side feature, не влияет на protocol

---

## 21. Premium Subscriptions (v3.0.0)

Модель подписок на premium-функции через periodic `ConsumeCredits`.

### 21.1 Premium Profile

- Стоимость: 10 TC/мес
- Benefits:
  - Verified badge в профиле (клиент-side флаг, не consensus)
  - Extended bio (до 2 KB вместо базовых 256 bytes)
  - Avatar высокого разрешения (до 512×512px вместо 128×128)
  - Custom status message (кратковременная строка «В отпуске до 15 мая»)
- Автоматическое продление: клиент публикует `ConsumeCredits` раз в месяц
- Cancel anytime: остановить auto-renewal в Settings → Subscriptions

### 21.2 Creator Subscriptions (платные каналы)

- Создатель публикует платный канал с ежемесячной подпиской
- Минимальная цена подписки: `creator_subscription_min_nj` (по умолчанию 0.1 TC/мес)
- Максимум — определяется создателем
- **Распределение платежа:** пока платёж = подписка полностью идёт в burn (за creator audit — в будущем возможно разделение 70% creator + 30% burn, но в v3.0.0 единая модель burn для простоты)
- Подписчик получает доступ к контенту канала; отсутствие оплаты в следующем месяце → отзыв доступа
- Клиент отслеживает active subscriptions и публикует monthly `ConsumeCredits`

### 21.3 Subscription Management UI

- Экран «Мои подписки» — список активных (премиум-профиль, каналы создателей)
- Для каждой: monthly cost, next renewal date, toggle auto-renew
- History past payments за N последних месяцев
...
- **Privacy by default.** Profile, encryption keys — всё опционально. Облачный fallback Juno выключен по умолчанию. Traffic camouflage включён по умолчанию.
...
**ProfileBlob, EncryptionKeyBlob, PreKeyBundle** — форматы описаны в существующих разделах app spec (см. разделы 7, 8).
