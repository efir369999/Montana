...
| Защита от Sybil | 13 000 окон VDF + invite slot | накопление окон требует работы в каждом окне |
...
Любой активный узел может стать кандидатом лотереи вне зависимости от chain_length — weighted_ticket нормализует по chain_length, и узел с малым chain_length может получить маленький ticket через случайность endpoint-а.
...
apply_proposal(state, proposal) -> state':
  ...
  Шаг 3: обработать expiry кандидатов и selection event.
    ...
    3b. Selection event (если current_window % 369 == 0):
        ...
        slots = max(1, floor(active_nodes(current_window) / 100))
...
5. proof_endpoint верифицируем: пересчёт VDF от `SHA-256("mt-candidate-vdf-init" || timechain_value(W_start) || node_id)` через 13 000 окон

Верификация: 13 000 сегментов VDF проверяются параллельно. На C ядрах: ~(13 000/C) × t_segment.
...
**Sybil-защита (три уровня):**
- VDF-барьер: 13 000 окон последовательного хэширования, 3 ядра CPU. Физическая работа.
- Selection rate limit: max 1% от active_nodes за 369 окон. Массовый вход ограничен.
- Weighted механизмы: chain_length определяет вес в лотерее и quorum. Новые узлы (chain_length = 1) имеют минимальное влияние. Время — единственный путь к весу.
...
Одно окно τ₁ регистрирует одну единицу Montana Time = 13 Ɉ. Число 60 — фиксированная конвенциональная константа протокола эмиссии за окно.
