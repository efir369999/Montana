...
```
endpoint_node(W) = SHA-256("mt-lottery" || T_r(W) || node_id || window_index)
```

Где T_r(W) — TimeChain VDF output окна W (каноничен, одинаков у всех узлов). Endpoint верифицируем за O(1) — один SHA-256.
...
VDF_Reveal:
  ...
  endpoint         32B     <- SHA-256("mt-lottery" || T_r || node_id || window_index)
...
| `mt-lottery` | Lottery endpoint seed (SHA-256(T_r \|\| node_id \|\| window_index)) |
...
| Ядра на узел | 1 | TimeChain VDF + validation (interleaved) |
