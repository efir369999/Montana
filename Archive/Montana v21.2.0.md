...
Пустой аккаунт без активности 56 дней — удаляется, кроме operator-аккаунтов узлов. Operator account защищён от pruning пока узел существует в Node Table — иначе TimeCoin-зачисление узлу-победителю некуда применить. Воссоздание обычного аккаунта: новый OpenAccount → аккаунт создаётся заново. Pruning детерминирован, автоматичен, каноничен.
...
|-----|----------|-----------|
| Transfer | Публичный перевод | FN-DSA-512 подпись, prev_hash, amount > 0, sender.balance >= amount, получатель существует |
...
OpenAccount:  создать запись в Account Table (balance = 0, pubkey, frontier_hash = H(operation))
...
**При финализации каждой операции** обновляется AccountChain length signer-аккаунта (подписавшего операцию):
...
Genesis State (height = -1, supply(-1) = 0):
  ...
  Genesis State Hash = SHA-256(Account Table || Node Table || protocol_params)
```
...
При финализации (apply step): если `Account Table[operator_account_id].is_node_operator == 1` на момент применения — отклонить (другая NodeRegistration финализирована раньше в canonical lex order). Иначе: создать запись в Node Table (start_window = текущее окно, operator_account_id зафиксирован), установить `is_node_operator = 1` у operator-аккаунта, очистить pending_invite, invite_window и invite_expires у пригласившего.
...
- Supply audit: Σ balance(account) для всех аккаунтов = 60_000_000_000 × (height + 1) nɈ
...
4. Catch-up cemented операций после τ₂ checkpoint:
   ...
   - Для каждой: проверить подпись FN-DSA-512, prev_hash, balance >= amount (для Transfer)
