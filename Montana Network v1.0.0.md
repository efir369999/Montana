...
**Слой:** Network (между Protocol и App)
- Зависимость снизу: Montana Protocol v35.24.0+ (state machine, crypto primitives, Genesis Decree)
- Зависимость сверху: Montana App v3.11.0+ (UI клиенты используют этот сетевой API)

---

## Введение

Сетевой слой Montana — транспорт и discovery между узлами консенсуса и клиентами. Эта спецификация была отделена от Montana Protocol v35.24.0 (где сетевой слой ранее жил inline разделами «Сетевой уровень» и далее) для разделения слоёв по принципу [I-7] минимальной криптографической поверхности и удобства независимого аудита.
...
Global invariant check:
  ...
  [I-5] Commodity hardware:              yes (1024 B frame × 1 fps = 1 KB/сек на
                                              исходящее соединение; 13 outbound × 1
...
Sensitivity: 8 inbound → 20 GB/мес; 16 inbound → 40 GB/мес — обе boundaries
             acceptable, 13 = средняя точка; ровно соответствует max_outbound
...
Следующие разделы извлечены из Montana App v3.11.0 разделов 10-11. Они описывают взаимодействие клиента с сетевым слоем — режимы узла, libp2p настройка с client-side, выбор хостящего узла, mesh integration на iOS/Android.
...
- **Montana Protocol** v35.24.0 (родительский слой) — state machine, crypto, consensus
- **Montana App** v3.11.0 (дочерний слой) — UI клиенты на основе этого сетевого API

## Версионная история

- **v1.0.0** — extracted из Montana Protocol v35.24.0 (разделы 4150-7409) + Montana App v3.11.0 (разделы 10-11). Initial separation для разделения слоёв и независимого аудита.

---

*Конец спецификации Montana Network v1.0.0.*
