...
**Bootstrap growth model.** Genesis пуст (0 аккаунтов). Первый AccountRecord создаётся при `settle(1)`: оператор, выигравший окно 0, получает первую эмиссию `EMISSION_moneta` на свой операторский аккаунт. Далее распространение Монтана через `Transfer` Mode B параллелизуется экспоненциально — каждый AccountRecord, накопивший положительный баланс и прошедший cooldown `1 Transfer Mode B per τ₂`, может создать новый AccountRecord:

- N = 0: 0 аккаунтов (пустой генезис); первый аккаунт — у победителя окна 0 при settle(1)
