...
**Version:** 35.28.0 (2026-06-25) — bootstrap growth model reconciled to the Selection event apply: the first operator AccountRecord is created at admission with balance 0; the first emission is credited at settle(1) to the already-existing account
...
Growth begins at the first cemented τ₁ window and requires no additional specialized mechanism — the existing `Transfer` Mode B rule plus the [I-15] cooldown covers the entire roll-out lifecycle.
...
Минимальный набор примитивов: `Transfer` (Mode A — перевод существующему, Mode B — перевод с созданием AccountRecord), `Anchor`, `ChangeKey`, `CloseAccount` — всё что нужно для канонического порядка событий и переводов ценности. Sequential time (SSHA iteration count, τ-окна, chain_length) — единственный consensus-critical cost.
