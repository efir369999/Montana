...
**Version:** 35.29.1 (2026-06-27) — account-creation terminology: an existing **sender** (not «sponsor») creates the receiver via `TransferActivation` (opcode 0x0A); `Transfer` (0x02) is top-up only (receiver must exist). Byte-exact to the reference implementation.
...
Два уровня входа в сеть. Узлы участвуют в консенсусе — открытый вход через SSHA + selection event, AccountRecord operator-а создаётся атомарно при cementing Selection event. Аккаунты пользователей держат и переводят средства — AccountRecord появляется операцией `TransferActivation` от существующего аккаунта-отправителя (создание + зачисление amount). Самоинициация создания аккаунта невозможна — требуется существующий отправитель.
