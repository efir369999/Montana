...
```
cd "/Users/kh./Python/Ничто/Montana/Russian/Протокол/Code" && cargo test -p mt-merkle -p mt-genesis -p mt-state -p mt-timechain
```
```
cd "/Users/kh./Python/Ничто/Montana/Russian/Протокол/Code" && wc -l crates/mt-{merkle,genesis,state,timechain}/src/lib.rs
```
```
cd "/Users/kh./Python/Ничто/Montana/Russian/Протокол/Code" && grep -E "pub const [A-Z_]+: &\[u8\]" crates/mt-codec/src/lib.rs | wc -l
```
```
cd "/Users/kh./Python/Ничто/Montana/Russian/Протокол/Code" && grep -rn "unsafe " crates/mt-{merkle,genesis,state,timechain}/src/
```
```
cd "/Users/kh./Python/Ничто/Montana/Russian/Протокол/Code" && grep -rn "f32\|f64" crates/mt-{merkle,genesis,state,timechain}/src/
```
```
cd "/Users/kh./Python/Ничто/Montana/Russian/Протокол/Code" && grep -n "pin 30/29\|inflation_num: 41" crates/mt-state/src/lib.rs crates/mt-genesis/src/lib.rs
```
