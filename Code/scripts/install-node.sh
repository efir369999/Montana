...
#   - auto-detects city/country/hosting from geo-IP (no IP published)
...
echo "[4/6] auto-detecting location (city/country/hosting from geo-IP)"
...
HOST="${MONTANA_HOSTING:-$(printf '%s' "$GEO" | sed -n 's/.*"isp":"\([^"]*\)".*/\1/p')}"
ALIAS="${MONTANA_ALIAS:-$(printf '%s' "${CITY:-node}" | tr 'A-Z ' 'a-z-' )}"
[ -n "$CITY" ] || CITY="$ALIAS"
echo "    alias=$ALIAS city=$CITY country=$CC hosting=$HOST"
...
docker run -d --name montana-node --network host --restart unless-stopped \
  ...
  -e MONTANA_ALIAS="$ALIAS" -e MONTANA_LABEL="$CITY" -e MONTANA_COUNTRY="$CC" -e MONTANA_HOSTING="$HOST" \
  "$IMAGE"
