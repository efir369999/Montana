...
## DEV-025 (open): target лотереи без τ₂-калибровки

`header.target = u128::MAX` (все активные узлы — кандидаты). Спецификация «Калибровка target» (integer-форма с TA-векторами) не реализована. До первой границы τ₂ (20160 окон) поведение совпадает со спецификацией при малой сети. Закрытие: реализация next_target в mt-lottery по биндинг-векторам TA1-TA3 + поле в settle.
