...
**Spec section:** «Genesis Decree» / «N_SEED как consensus-binding параметр Genesis Decree» (Montana Protocol v35.26.1)
...
**Spec quote:** «`winner_{W-1} = argmin(weighted_ticket_node)` среди cemented `SshaReveal` узлов-кандидатов окна W-1»
...
**Status:**          partial — closed конструкцией: (d) TTL — current_window() из системных часов (floor(unix/60), off-chain [P2P-1]) в handle_deposit + prune-таймер (60s) в PostmanServer::run эвиктит истёкшие шарды (было window=0 static → TTL мёртв). closed конструкцией также: (a) drop-on-ack §593 — handle_relay_subscribe НЕ дропает при send курьеру; TAG_RECEIVE_ACK/RELAY_ACK + mt_client_ack + ack_drain: буфер держится до E2E-подтверждения B. ЗАКРЫТ ПОЛНОСТЬЮ. [legacy] (a) drop-on-delivery по E2E receipt §593; (b) RS(k,n) фан-аут §201/§508 по k хостам — multi-host deposit + m-of-k reconstruct; (c) padding-to-bucket §490 — E2E-слой мессенджера (bucket_len helper готов в mt-overlay, применяет мессенджер перед E2E-encrypt, transport ct opaque). (a)+(b)+(c) — фундаментальный durability redesign > 1 дня, отдельный milestone. ГРАНИЦА (P2P Network v0.14.1 §508): (a) drop-on-delivery стирает ТОЛЬКО транзитный буфер почтальона после E2E-подтверждения B — НЕ историю; персистентная история/медиа резервируется облаком телефонов (Montana Messenger «Резервный субстрат данных», k-из-n R=2, отдельный слой), drop транзита её не касается. (b) RS транзита и k-из-n облака — одна erasure-машинерия на двух слоях.
**Acknowledged:**    deep critic-audit (2026-07-14); (d) closed per author 'закрывай до конца' (2026-07-15); (a)+(b)+(c) narrowed honestly as cross-layer durability milestone
