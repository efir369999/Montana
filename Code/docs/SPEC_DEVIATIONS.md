...
**Что делает код:** текущая `mt-net::ibt::online_proof` использует формулу до bump — `sign(sk, "mt-tunnel-online" || server_node_id || floor(W/2))` без `online_session_nonce`. Wire format не содержит plain nonce field в advertisement.
**Severity:** средний (Pre-mainnet — нет deployed network в multi-node режиме, поэтому wire mismatch не affecting production; spec ahead of code — норма для phased close)
**Closure path:**
  1. `mt-net::ibt::online_proof(sk, server_node_id, window_index, nonce)` — добавить 4-й параметр `nonce: [u8; 32]`; включить в подписанное сообщение.
  2. `mt-net::ibt::verify_online_proof` — принять nonce, добавить domain-extended message.
  3. `mt-net::ibt::OnlineHandshake` или эквивалентный struct envelope — добавить plain `online_session_nonce: [u8; 32]` field; sender генерирует через `OsRng`, receiver валидирует не-replay через `used_online_nonces[client_pubkey]` set.
  4. `used_online_nonces` lifecycle: in-memory `HashMap<PublicKey, HashSet<[u8; 32]>>` либо `lru::LruCache` с pruning по window slot expiry (≤2 окна); per-pubkey set bounded.
  5. KAT B1 в `crates/mt-net/tests/test_vectors.rs::vector_b1` — regen `sha256(proof signature 3309B)` через actual reference impl после param add.
  6. Sync TBD-A marker в `Montana Network v1.1.0.md` Vector B1 → real hex после regen.
**Closure cost:** ~0.5 рабочего дня (param add + verify update + nonce tracking struct + KAT regen)
**Status:** открыто

**Acknowledged:** Spec patch MONT-002 (Metzdowd consolidated audit 2026-05-19), commit d33a9b5 — spec bumped Network v1.0.0 → v1.1.0 first, code follows in next mt-net Phase B sync. Per Code/CLAUDE.md Pre-mainnet принцип для кода — spec first, no silent divergence.
