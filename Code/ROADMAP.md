...
| 6 | `mt-timechain` | M2 | mt-codec, mt-crypto, mt-genesis, mt-state | TimeChain SSHA + Adaptive D + cemented_bundle_aggregate | ✅ Committed (76ed8da) |
...
| 10 | `mt-entry` | M4 | mt-state, mt-timechain | NodeRegistration + selection event + adaptive SSHA | ✅ Closed (5 phases, 39 тестов) |
...
- [x] `mt-timechain` — ssha_step/verify (T_r = SHA-256^D), next_d (Adaptive D ±3%), cemented_bundle_aggregate (unpredictable-offline binding, 3 ветви). 23 теста. Commit `76ed8da`.
...
Раздел спеки: «SSHA Reveal и лотерея» (~строка 754+), «Confirmer threshold», «cemented_bundle_aggregate» (уже в mt-timechain).
...
- **Phase B. SSHA_Reveal тип + layout** ✅ (15 тестов)
  - `struct SshaReveal { node_id, window_index, endpoint, signature }` — REVEAL_SIZE = 734 B
  - CanonicalEncode + encode_signed_scope (SSI R1), reveal_hash = identifier (SSI R2, class "mt-ssha-reveal")
  ...
  - validate_reveal: спек-правила 1, 2, 3, 5 («Валидация SSHA_Reveal», строки 1020-1026). Правило 4 (weighted_ticket < target) — в Phase C
...
Раздел спеки: «Вход и регистрация», «apply_proposal Шаг 1/3a/3b», «Adaptive SSHA».
...
- **Phase A. NodeRegistration** ✅ (11 тестов)
  ...
  - NodeRegError: UnsupportedSuite, InvalidSignature, NodeIdAlreadyIn{NodeTable,CandidatePool}, OperatorAccount{NotFound,AlreadyNode}, WStartOutOfRange, SshaChainTooShort
  - validate_noderegistration (structural checks 1-3)
- **Phase B. candidate_ssha_init + Candidate Pool** ✅ (4 теста)
  - `candidate_ssha_init(t_r, cba, node_id)` = SHA-256("mt-candidate-ssha-init" || ...)
...
- **Phase D. Adaptive SSHA** ✅ (6 тестов)
  - `required_ssha_length(pending, active, τ₂)` — integer form через permille (per [I-9])
...
| 2. M2 TimeChain + State | `mt-examples/examples/m2_timechain_state.rs` | ✅ passed (5/5 PASS — ssha-forward 1000, next-d-boundaries, cba-branches, state-root-compose, merkle-inclusion; binary написан 2026-04-30 коммитом `7e333e6`, прогнан автором на iMac) | 2026-04-30 |
...
Что проверяем:
- SSHA forward: 1000 steps от genesis T_r_0, сверить T_r_1000 ручным пересчётом `SHA-256^1000`
...
Команды:
```bash
cd "..." && cargo run --release --example m2_timechain_state -- ssha-forward 1000
...
Critic checklist:
- SSHA backward — impossible? (должен — preimage resistance SHA-256)
...
| 1.1.3 | `CanonicalEncode` trait impls для всех consensus types | mt-codec/src/lib.rs + per-crate impl | `grep "impl CanonicalEncode"` workspace-wide | каждый consensus type implements | NodeRegistration, ProposalHeader, BundledConfirmation, SshaReveal, AccountRecord, NodeRecord, CandidateRecord — все present |
...
#### M2.4 mt-timechain — SSHA + adaptive D + cba

| # | Элемент | file:line | Reproduce | Expected | Pass |
|---|---------|-----------|-----------|----------|------|
| 2.4.1 | `ssha_step(prev, d)` SHA-256^d associative | mt-timechain/src/lib.rs:13 | composition test: `ssha_step(ssha_step(x, a), b) == ssha_step(x, a+b)` | associativity holds | invariant PASS |
| 2.4.2 | `ssha_step(prev, 0) == prev` identity | mt-timechain | zero-iteration check | output == input | unit-test PASS |
| 2.4.3 | `ssha_verify(prev, expected, d)` | mt-timechain::ssha_verify | sign correct + tamper | accept correct, reject wrong claim | 2/2 cases PASS |
...
| 2.4.9 | TimeChain SSHA на genesis узле тикает per окно | montana-node start | `montana-node start --max-windows 2 && status` | T_r изменился, current_window инкрементировано на 2 | observable PASS |
...
| 3.16 | `supply_moneta(W, params) = emission × W` closed-form | mt-account::supply_moneta | различные W | linear formula | unit-test PASS |
...
#### M4.1 mt-lottery — BundledConfirmation + SshaReveal + lottery
...
| 4.1.6 | `SshaReveal` layout 32+8+32+SIG bytes | mt-lottery::REVEAL_SIZE | size check | == REVEAL_SIZE const | static |
| 4.1.7 | `reveal_hash(reveal)` SHA-256("mt-ssha-reveal" \|\| scope) | mt-lottery::reveal_hash | external oracle | byte-exact | external PASS |
...
| 4.3.4 | `candidate_ssha_init([T_r, cba(W-2), node_id])` [I-8] | mt-entry::candidate_ssha_init | external oracle | byte-exact composition | external PASS |
...
| 4.3.11 | `apply_noderegistrations_batch` incremental sort+apply | mt-entry::apply_noderegistrations_batch | per-window | applied count == 1 if ssha_chain_length ≥ τ₂ | observable PASS |
| 4.3.12 | `required_ssha_length(pending, active, τ₂)` integer permille | mt-entry::required_ssha_length | pending=0,active=0 → tau2_windows | spec-binding | unit-test PASS |
...
| 6.16 | Candidate path (post-ceremony, mismatch pubkey) | node_lifecycle.rs:78 | finalized bootstrap_node_pubkey != identity.node_pk | phase = Bootstrap → CandidateSsha на первом окне | DEV-010 logic PASS (не тестируется до ceremony) |
...
Pre-mainnet упрощение монетарной политики: pin 41/40 + carry-recurrence заменён на единственную константу `EMISSION_moneta = 13 × 10⁹ nɈ` per окно. `reward_moneta(W) = EMISSION_moneta` (const, навсегда), `supply_moneta(W) = EMISSION_moneta × W` closed-form.
...
- `supply_moneta(W, params)` — closed-form `emission × W`
...
| 2026-04-20 | M4 + M5 CLOSED: mt-consensus (60 тестов, 5 phases — header + Lookback + control_set + Canonical acceptance + Finalization) + mt-entry (39 тестов, 5 phases — NodeRegistration + Candidate Pool + Selection event + Adaptive SSHA + batch apply) + mt-store (24 теста, 5 phases — FsStore + table persist + proposal archive + crash recovery + pruning). Workspace 506 тестов. Next: Manual Validation Gate (5 сценариев). | (предыдущий) |
...
| 2026-04-19 | M4 Phase B: mt-lottery SshaReveal тип + encode/signed_scope/reveal_hash/compute_endpoint/validate_reveal (15 тестов) | 5395581 |
