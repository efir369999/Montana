...
**Version:** 36.0.0 (2026-07-10) — unified device model. Every device is a node (two forms: application / terminal). The continuous SSHA pendulum is removed: one window = D sequential hashes computed once by the window's proposer (the sprint). Emission is divided equally among the window's living participants («lived = acted»); the emission lottery is removed — a draw selects only the proposer role. Entry of a new identity: 1 per acting account per τ₂ + the continuity gate (14 consecutive segments of life). New Genesis (protocol_params layout changed).
...
dust_carry:      u128 LE — копеечный невыданный остаток эмиссионного котла (см. «Анти-инфляция»);
                 входит в state_root как пятый компонент.
...
apply_proposal(state, proposal) -> state':
                      ...
                      применить incremental apply в окне W_p:
                        ...
                        for each NR in sorted order:
                          ...
                          if НЕ (младшие required_segments бит effective_bitmap все установлены):
                            ...
                            continue
                          # Genesis-исключение: при пустой Node Table гейт не применяется (первый вход).
  ...
  Шаг 2: эмиссия окна — поровну живущим.
    ...
    N_present = |living(W)|        # ≥ 1 по построению (окно ⟺ финализация ⟺ живущие)
...
Genesis State (до первого окна, supply = 0):
  ...
  protocol_params (каноническая сериализация, little-endian, фиксированная длина полей):
    ...
    continuity_segments            (2B)   14 (число последовательных сегментов гейта непрерывности; segment_windows = τ₂ / 14 = 1440)
...
for each NR in order:
  ...
  if continuity(NR.operator_account) >= required:
